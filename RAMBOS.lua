...
--- Start Source By Rami syria »»»»»»»
local tdcli = dofile("tdcli.lua")
local RMJS = dofile('./JSON.lua')
...
local RmOwner_ = tonumber(io.read())
if not tostring(RmOwner_):match('%d+') then
RmOwner_ = 280911803
end
io.write("\27[0;36m>>"..[[
ارسل توكن البوت : ]].."\n\027[00m")
...
print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m\n➡➡[•• اكتمل تكوين الـconfig ••]▶\n\027[00m")
end
create_config_auto()

file = io.open("RAMBOS", "w")
file:write([[
token="]]..Token_..[["
COUNTER=1
while(true) do
...
file:close()
sleep(1)
os.execute(' screen -S rambos ./rambos')
end
...
RAMBOS_SUDO = sudos.RmOwner   -------bot owner
RAMBOS_TOKEN = sudos.Token -----RAMBOS_TOKEN
bot_id = sudos.Bot_ID ---id bot
RAMBOS = tonumber(_redis.Bot_ID)

function Run()
print('\27[93m>Developer:\27[39m'..' '..'@th3victory')
end
------------------------boT ID   BY rami ----------------------
print(string.sub(_redis.Bot_ID,1,0))
local bot_id = redis:get(RAMBOS.."Bot:rmBotAccount") or tonumber(_redis.Bot_ID)
...
redis:sadd(RAMBOS.."Bot:RmSudos", user)
end
redis:setex(RAMBOS.."bot:reload", 7230, true)
redis:set(RAMBOS.."Bot:RmOwnerBot", config.RmOwner or 0)
redis:set(RAMBOS.."Bot:Run", config.Run or 0)
local Api = config.Token:match("(%d+)")
local RD = RNM or 0
if Api then
redis:set(RAMBOS.."Bot:Api_ID", Api)
end
function AuthoritiesEn()
local hash = "Bot:RmSudos"
local list = redis:smembers(RAMBOS..hash)
local OwnerRM_ = redis:get(RAMBOS.."Bot:RmOwnerBot")
local text = "List of Authorities :\n"
local user_info_ = redis:get(RAMBOS.."user:Name" .. OwnerRM_)
...
local user_info = redis:get(RAMBOS.."user:Name" .. v)
if user_info then
...
local list2 = redis:smembers(RAMBOS..hash2)
if #list2 ~= 0 then
text = text .. [[
...
redis:set(RAMBOS.."AuthoritiesEn", text)
end
...
redis:set(RAMBOS.."AuthoritiesFa", text)
end
AuthoritiesEn()
AuthoritiesFa()
end
...
redis:del(RAMBOS.."Bot:RmSudos")
redis:del(RAMBOS.."Bot:RmOwnerBot")
redis:del(RAMBOS.."Bot:Token")
redis:del(RAMBOS.."Bot:Channel")
setdata()
end
...
usr = string.gsub(usr, "%s+$", "")
usr = string.gsub(usr, "[\n\r]+", " ")
redis:set(RAMBOS.."Bot:ServerUser", usr)----------ServerUser
redis:del(RAMBOS.."MatchApi")
redis:del(RAMBOS.."Set_Our_ID")
redis:del(RAMBOS.."Open:Chats")
local rmdata = redis:get(RAMBOS.."Botid" .. bot_id) or "\n"
local Botrmdata = redis:get(RAMBOS.."RmOwnerBot" .. config.RmOwner) or "\n"
if redis:get(RAMBOS.."Rank:Data") then
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. rmdata .. "\027[00m")
print("\027[" .. color.yellow[1] .. ";" .. color.black[2] .. "m" .. Botrmdata .. "\027[00m")
for v, user in pairs(config.Sudo_Users) do
local SudoData = redis:get(RAMBOS.."RmSudos" .. user)
if SudoData then
...
redis:set(RAMBOS.."Our_ID", result.id_)
end
...
local lists = redis:smembers(RAMBOS..hashs)
redis:del(RAMBOS.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(RAMBOS.."SudoNumberGp" .. user_id)
end
end
local setnumbergp_three = function(user_id)
local hashss = "sudo:data:" .. user_id
local lists = redis:smembers(RAMBOS..hashss)
redis:del(RAMBOS.."SudoNumberGp" .. user_id)
for k, v in pairs(lists) do
redis:incr(RAMBOS.."SudoNumberGp" .. user_id)
end
end
local list = redis:smembers(RAMBOS.."Bot:Admins")
for k, v in pairs(list) do
setnumbergp_two(v)
end
local lists = redis:smembers(RAMBOS.."Bot:RmSudos")
for k, v in pairs(lists) do
setnumbergp_three(v)
end
redis:setex(RAMBOS.."bot:reload", 7230, true)
...
local Bot_Channel = redis:get(RAMBOS.."Bot:Channel") or tostring(_redis.Channel)
local sudo_users = _config.Sudo_Users
local Rm_Owner = redis:get(RAMBOS.."Bot:RmOwnerBot")
local run = redis:get(RAMBOS.."Bot:Run") or "True"
if not redis:get(RAMBOS.."setnumbergp") then
setnumbergp()
redis:setex(RAMBOS.."setnumbergp", 5 * hour, true)
end
...
print("\27[0;35m>>"..[[
تم كتابه السورس بوسطه
الـدولـة ابـن الاسـد
WRITING THE SOURCE BY : @RAMBO_SYR
CH SOURCE : @TH3VICTORY
]].."\n\027[00m")
-----------------------function is_RM----------by rami------------------------
local is_RM = function(msg)
local var = false
if msg.sender_user_id_ == tonumber(Rm_Owner) then
var = true
end
return var
end
local is_RMiD = function(user_id)
local var = false
if user_id == tonumber(Rm_Owner) then
var = true
end
return var
end
...
local var = false
if redis:sismember(RAMBOS.."Bot:RmSudos", msg.sender_user_id_) then
var = true
end
if msg.sender_user_id_ == tonumber(Rm_Owner) then
var = true
...
local var = false
if redis:sismember(RAMBOS.."Bot:RmSudos", user_id) then
var = true
end
if user_id == tonumber(Rm_Owner) then
var = true
...
local admin = redis:sismember(RAMBOS..hashsb, user_id)
if admin then
var = true
end
if redis:sismember(RAMBOS.."Bot:RmSudos", user_id) then
...
local monshi = redis:sismember(RAMBOS..hashssk, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(RAMBOS..hashs, user_id)
if monshi then
var = true
end
if admin then
...
local owner = redis:sismember(RAMBOS..hash, user_id)
if monshi then
var = true
end
if admin then
...
local momod = redis:sismember(RAMBOS..hash, user_id)
local hashs = "Bot:Admins"
local admin = redis:sismember(RAMBOS..hashs, user_id)
local hashssk = "bot:monshis:" .. chat_id
local monshi = redis:sismember(RAMBOS..hashssk, user_id)
local hashss = "bot:owners:" .. chat_id
local owner = redis:sismember(RAMBOS..hashss, user_id)
local our_id = redis:get(RAMBOS.."Our_ID") or 0
if momod then
var = true
end
if owner then
...
local vipmem = redis:sismember(RAMBOS..hashsss, user_id)
if vipmem then
var = true
end
if momod then
...
local vipmems = redis:sismember(RAMBOS..hashsss, user_id)
if vipmems then
var = true
end
if redis:sismember(RAMBOS.."Bot:RmSudos", user_id) then
...
local banned = redis:sismember(RAMBOS..hash, user_id)
if banned then
var = true
end
return var
...
local var = false
local hash = "bot:gban:"
local gbanned = redis:sismember(RAMBOS..hash, user_id)
if gbanned then
var = true
end
return var
...
local muted = redis:sismember(RAMBOS..hash, user_id)
local muted2 = redis:get(RAMBOS..hash2)
if muted then
var = true
end
if muted2 then
...
if redis:get(RAMBOS.."GetTime") then
local data = redis:get(RAMBOS.."GetTime")
...
redis:setex(RAMBOS.."GetTime", 10, url)
local A = jdat.FAtime
local B = jdat.FAdate
if A and B then
local T = {time = A, date = B}
...
------------------function Join CH------------------------------------------
function rm_JoinCh(msg)
local var = true
if redis:get(RAMBOS.."rmjoin1") then
local channel = ''..redis:get(RAMBOS..'rmch1')..''
local url , res = https.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/getchatmember?chat_id='..channel..'&user_id='..msg.sender_user_id_)
local data = RMJS:decode(url)
...
local tmRAMBOS = function(msg)
if is_RMiD(msg.sender_user_id_) then
RAMBOS  = "المطور  👨‍✈️"
...
redis:set(RAMBOS..'group:name'..GroupID,data.title_) end,nil) return redis:get(RAMBOS..'group:name'..GroupID) end
...
link = redis:get(RAMBOS.."bot:group:link" .. chat)
if link then
...
redis:del(RAMBOS.."bot:muteall" .. chat_id)
redis:del(RAMBOS.."bot:text:mute" .. chat_id)
redis:del(RAMBOS.."bot:photo:mute" .. chat_id)
redis:del(RAMBOS.."bot:video:mute" .. chat_id)
redis:del(RAMBOS.."bot:selfvideo:mute" .. chat_id)
redis:del(RAMBOS.."bot:gifs:mute" .. chat_id)
redis:del(RAMBOS.."anti-flood:" .. chat_id)
redis:del(RAMBOS.."flood:max:" .. chat_id)
redis:del(RAMBOS.."bot:sens:spam" .. chat_id)
redis:del(RAMBOS.."floodstatus" .. chat_id)
redis:del(RAMBOS.."bot:music:mute" .. chat_id)
redis:del(RAMBOS.."bot:bots:mute" .. chat_id)
redis:del(RAMBOS.."bot:duplipost:mute" .. chat_id)
redis:del(RAMBOS.."bot:inline:mute" .. chat_id)
redis:del(RAMBOS.."bot:cmds" .. chat_id)
redis:del(RAMBOS.."bot:bots:mute" .. chat_id)
redis:del(RAMBOS.."bot:voice:mute" .. chat_id)
redis:del(RAMBOS.."editmsg" .. chat_id)
redis:del(RAMBOS.."bot:links:mute" .. chat_id)
redis:del(RAMBOS.."bot:pin:mute" .. chat_id)
redis:del(RAMBOS.."bot:sticker:mute" .. chat_id)
redis:del(RAMBOS.."bot:tgservice:mute" .. chat_id)
redis:del(RAMBOS.."bot:webpage:mute" .. chat_id)
redis:del(RAMBOS.."bot:strict" .. chat_id)
redis:del(RAMBOS.."bot:hashtag:mute" .. chat_id)
redis:del(RAMBOS.."tags:lock" .. chat_id)
redis:del(RAMBOS.."bot:location:mute" .. chat_id)
redis:del(RAMBOS.."bot:contact:mute" .. chat_id)
redis:del(RAMBOS.."bot:english:mute" .. chat_id)
redis:del(RAMBOS.."bot:arabic:mute" .. chat_id)
redis:del(RAMBOS.."bot:forward:mute" .. chat_id)
redis:del(RAMBOS.."bot:member:lock" .. chat_id)
redis:del(RAMBOS.."bot:document:mute" .. chat_id)
redis:del(RAMBOS.."markdown:lock" .. chat_id)
redis:del(RAMBOS.."Game:lock" .. chat_id)
redis:del(RAMBOS.."bot:spam:mute" .. chat_id)
redis:del(RAMBOS.."post:lock" .. chat_id)
redis:del(RAMBOS.."bot:welcome" .. chat_id)
redis:del(RAMBOS.."delidstatus" .. chat_id)
redis:del(RAMBOS.."delpro:" .. chat_id)
redis:del(RAMBOS.."sharecont" .. chat_id)
redis:del(RAMBOS.."sayedit" .. chat_id)
redis:del(RAMBOS.."welcome:" .. chat_id)
redis:del(RAMBOS.."bot:group:link" .. chat_id)
redis:del(RAMBOS.."bot:filters:" .. chat_id)
redis:del(RAMBOS.."bot:muteall:Time" .. chat_id)
redis:del(RAMBOS.."bot:muteall:start" .. chat_id)
redis:del(RAMBOS.."bot:muteall:stop" .. chat_id)
redis:del(RAMBOS.."bot:muteall:start_Unix" .. chat_id)
redis:del(RAMBOS.."bot:muteall:stop_Unix" .. chat_id)
redis:del(RAMBOS.."bot:muteall:Run" .. chat_id)
redis:del(RAMBOS.."bot:muted:" .. chat_id)
end
...
redis:set(RAMBOS.."bot:webpage:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:inline:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:bots:mute" .. chat_id, true)
redis:set(RAMBOS.."tags:lock" .. chat_id, true)
redis:set(RAMBOS.."markdown:lock" .. chat_id, true)
redis:set(RAMBOS.."bot:links:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:hashtag:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:spam:mute" .. chat_id, true)
redis:set(RAMBOS.."anti-flood:" .. chat_id, true)
redis:set(RAMBOS.."Game:lock" .. chat_id, true)
redis:set(RAMBOS.."bot:panel" .. chat_id, "one")
end
...
redis:set(RAMBOS.."anti-flood:" .. chat_id, true)
redis:set(RAMBOS.."Game:lock" .. chat_id, true)
redis:set(RAMBOS.."post:lock" .. chat_id, true)
redis:set(RAMBOS.."bot:forward:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:photo:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:video:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:selfvideo:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:gifs:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:sticker:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:location:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:document:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:panel" .. chat_id, "two")
end
...
redis:set(RAMBOS.."bot:links:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:spam:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:sens:spam" .. chat_id, 500)
redis:set(RAMBOS.."anti-flood:" .. chat_id, true)
redis:set(RAMBOS.."Game:lock" .. chat_id, true)
redis:set(RAMBOS.."bot:cmds" .. chat_id, true)
redis:set(RAMBOS.."bot:duplipost:mute" .. chat_id, true)
redis:set(RAMBOS.."bot:panel" .. chat_id, "three")
end
...
local user_id
local user = false
local list1 = redis:smembers(RAMBOS.."Bot:RmSudos")
local list2 = redis:smembers(RAMBOS.."Bot:Admins")
for k, v in pairs(list1) do
local hash = "sudo:data:" .. v
local is_add = redis:sismember(RAMBOS..hash, chat)
if is_add then
user_id = v
end
end
...
local hash = "sudo:data:" .. Rm_Owner
if redis:sismember(RAMBOS..hash, chat) then
user_id = Rm_Owner
end
if user_id then
local user_info = redis:get(RAMBOS.."user:Name" .. user_id)
if user_info then
user = user_info
end
end
...
if hash then
local names = redis:hkeys(RAMBOS..hash)
local text = ""
for i = 1, #names do
...
redis:set(RAMBOS.."bot:Uptime", os.time())
----------------tdcli_update_callback---------------------------------------------------------------------------
function tdcli_update_callback(data)
local our_id = redis:get(RAMBOS.."Our_ID") or 0
local api_id = redis:get(RAMBOS.."Bot:Api_ID") or 0
if data.ID == "UpdateNewMessage" then
local msg = data.message_
local d = data.disable_notification_
local chat = chats[msg.chat_id_]
redis:sadd(RAMBOS.."groups:users" .. msg.chat_id_, msg.sender_user_id_)--save users gp
redis:incr(RAMBOS.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_.."")--save msgs gp
if msg.content_.ID == "MessageChatDeleteMember" then
if tonumber(msg.content_.user_.id_) == tonumber(_redis.Bot_ID) then
local user_info_ = redis:get(RAMBOS.."user:Name" .. msg.sender_user_id_)
local UserRAMBOS = user_info_
if user_info_ then 
local sudoed = tonumber(Rm_Owner)
local iD_RAMBOS = [[
...
✓‏
‌‏]]
send(sudoed, 0, 1,iD_RAMBOS, 1, "md")
redis:del(RAMBOS.."bot:enable:" .. msg.chat_id_)
redis:srem(RAMBOS.."bot:groups", msg.chat_id_)
end end end
if msg.content_.ID == "MessageChatAddMembers" then
redis:incr(RAMBOS..'rmaddcon'..msg.chat_id_..':'..msg.sender_user_id_)
if msg.date_ < os.time() - 40 then
print("\027[" .. color.white[1] .. ";" .. color.magenta[2] .. "m>>> OLD MSG <<<\027[00m")
return false
end
if not redis:get(RAMBOS.."Set_Our_ID") then
...
redis:set(RAMBOS.."RmSudos" .. user, "> Sudo User ID : " .. sudouserid .. [[
...
redis:set(RAMBOS.."RmOwnerBot" .. Rm_Owner, "> Bot Owner ID : " .. botowneruserid .. [[
...
getUser(Rm_Owner, outputbotowner)
--------------------function outputbot----------------------------------------
...
redis:set(RAMBOS.."Botid" .. result.id_, "> Bot ID : " .. botuserid .. [[
...
redis:setex(RAMBOS.."Rank:Data", 700, true)
end
if redis:get(RAMBOS.."bot:reload") and 30 > tonumber(redis:ttl(RAMBOS.."bot:reload")) then
load_config()
setnumbergp()
redis:setex(RAMBOS.."bot:reload", 7230, true)
print("\027[" .. color.black[1] .. ";" .. color.green[2] .. "m>>> Bot Reloaded <<<\027[00m")
end
if not redis:get(RAMBOS.."bot:reload2") then
redis:del(RAMBOS.."bot:groups")
redis:del(RAMBOS.."bot:userss")
redis:setex(RAMBOS.."bot:reloadingtime", 22 * hour, true)
redis:setex(RAMBOS.."bot:reload2", week, true)
redis:setex(RAMBOS.."bot:reload3", 2222 * day, true)
redis:setex(RAMBOS.."bot:reload4", 2222 * day, true)
end
if redis:get(RAMBOS.."bot:reload3") and 500 >= tonumber(redis:ttl(RAMBOS.."bot:reload3")) then
local hash = "bot:groups"
local list = redis:smembers(RAMBOS..hash)
for k, v in pairs(list) do
if not redis:get(RAMBOS.."bot:enable:" .. v) and not redis:get(RAMBOS.."bot:charge:" .. v) then
resetgroup(v)
chat_leave(v, bot_id)
redis:srem(RAMBOS..hash, v)
end
end
redis:del(RAMBOS.."bot:reload3")
end
if redis:get(RAMBOS.."bot:reload4") and redis:ttl(RAMBOS.."bot:reload4") <= 600 then
local reload_data_sudo = function()
local hashsudo = "Bot:RmSudos"
local listsudo = redis:smembers(RAMBOS..hashsudo)
for k, v in pairs(listsudo) do
local hashdata = "sudo:data:" .. v
local listdata = redis:smembers(RAMBOS..hashdata)
for k, gp in pairs(listdata) do
if not redis:sismember(RAMBOS.."bot:groups", gp) then
redis:srem(RAMBOS..hashdata, gp)
end
end
end
end
...
local listadmin = redis:smembers(RAMBOS..hashadmin)
...
------------------------------EXpirepannel GP ----------------------------------------------------------
local expiretime = redis:ttl(RAMBOS.."bot:charge:" .. msg.chat_id_)
if not redis:get(RAMBOS.."bot:charge:" .. msg.chat_id_) and redis:get(RAMBOS.."bot:enable:" .. msg.chat_id_) then
redis:del(RAMBOS.."bot:enable:" .. msg.chat_id_)
redis:srem(RAMBOS.."bot:groups", msg.chat_id_)
end
if redis:get(RAMBOS.."bot:charge:" .. msg.chat_id_) and not redis:get(RAMBOS.."bot:enable:" .. msg.chat_id_) then
redis:set(RAMBOS.."bot:enable:" .. msg.chat_id_, true)
end
if not redis:get(RAMBOS.."bot:expirepannel:" .. msg.chat_id_) and redis:get(RAMBOS.."bot:charge:" .. msg.chat_id_) and tonumber(expiretime) < tonumber(day) and tonumber(expiretime) >= 3600 then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
local v = tonumber(Rm_Owner)
local list = redis:smembers(RAMBOS.."bot:owners:" .. msg.chat_id_)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(RAMBOS.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
owner = user_info
else
...
send(v, 0, 1, "💲┊ سوف تنتهي صلاحيه المجموعه\n🔅┊الرابط : " .. (redis:get(RAMBOS.."bot:group:link" .. msg.chat_id_) or "لا يوجد ") .. "\n🚫┊ الايدي » " .. msg.chat_id_ .. "", 1, "html")
redis:setex(RAMBOS.."bot:expirepannel:" .. msg.chat_id_, 43200, true)
end
end
------------------------Autoleave FOR BOT----------------------------------------------------
if redis:get(RAMBOS.."autoleave") == "On" then
local id = tostring(msg.chat_id_)
if not redis:get(RAMBOS.."bot:enable:" .. msg.chat_id_) and id:match("-100(%d+)") and not redis:get(RAMBOS.."bot:autoleave:" .. msg.chat_id_) then
redis:setex(RAMBOS.."bot:autoleave:" .. msg.chat_id_, 1400, true)
end
local autoleavetime = tonumber(redis:ttl(RAMBOS.."bot:autoleave:" .. msg.chat_id_))
local time = 400
if tonumber(autoleavetime) < tonumber(time) and tonumber(autoleavetime) > 150 then
redis:set(RAMBOS.."lefting" .. msg.chat_id_, true)
end
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") and redis:get(RAMBOS.."lefting" .. msg.chat_id_) then
if not redis:get(RAMBOS.."bot:enable:" .. msg.chat_id_) and not redis:get(RAMBOS.."bot:charge:" .. msg.chat_id_) then
redis:del(RAMBOS.."lefting" .. msg.chat_id_)
redis:del(RAMBOS.."bot:autoleave:" .. msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
local v = tonumber(Rm_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعــه\n🔱┊ الاسم » 👇🏾\n🏮┊ ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(RAMBOS.."bot:groups", msg.chat_id_)
elseif redis:get(RAMBOS.."bot:enable:" .. msg.chat_id_) then
redis:del(RAMBOS.."lefting" .. msg.chat_id_)
end
end
elseif redis:get(RAMBOS.."bot:charge:" .. msg.chat_id_) == "Trial" and 500 > redis:ttl(RAMBOS.."bot:charge:" .. msg.chat_id_) then
local v = tonumber(Rm_Owner)
send(v, 0, 1, "💲┊ تم مغادره المجموعــه\n🔱┊ الاسم » ("..title_name(msg.chat_id_)..")\n🚫┊ الايدي » " .. msg.chat_id_, 1, "html")
redis:srem(RAMBOS.."bot:groups", msg.chat_id_)
chat_leave(msg.chat_id_, bot_id)
redis:del(RAMBOS.."bot:charge:" .. msg.chat_id_)
end
...
redis:set(RAMBOS..svgroup, chatname)
end
end
...
local id = result.id_
if username then
redis:set(RAMBOS..svuser, "@" .. username)
else
redis:set(RAMBOS..svuser, name)
end
end
getUser(msg.sender_user_id_, check_username)
if redis:get(RAMBOS.."clerk") == "On" then
local clerk = function(extra, result)
if not is_admin(result.id_) then
local textc = redis:get(RAMBOS.."textsec")
if not redis:get(RAMBOS.."secretary_:" .. msg.chat_id_) and textc then
...
redis:setex(RAMBOS.."secretary_:" .. msg.chat_id_, day, true)
end
end
end
...
local status_welcome = (redis:get(RAMBOS..'status:welcome:'..msg.chat_id_) or 'disable')
...
if redis:get(RAMBOS..'welcome:'..msg.chat_id_) then
text = redis:get(RAMBOS..'welcome:'..msg.chat_id_)
else
...
----------------- save all msg bot --------------------------
redis:incr(RAMBOS.."bot:allmsgs")
if msg.chat_id_ then
local id = tostring(msg.chat_id_)
if id:match("-100(%d+)") then
if not redis:sismember(RAMBOS.."bot:groups", msg.chat_id_) then
redis:sadd(RAMBOS.."bot:groups", msg.chat_id_)
end
elseif id:match("^(%d+)") then
if not redis:sismember(RAMBOS.."bot:userss", msg.chat_id_) then
redis:sadd(RAMBOS.."bot:userss", msg.chat_id_)
end
elseif not redis:sismember(RAMBOS.."bot:groups", msg.chat_id_) then
redis:sadd(RAMBOS.."bot:groups", msg.chat_id_)
end
end
---------------type the msg--------------------------------------------------
...
redis:incr(RAMBOS.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")
text = msg.content_.text_
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Text ] <<\027[00m")
msg_type = "MSG:Text"
end
if msg.content_.ID == "MessagePhoto" then
redis:incr(RAMBOS.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(RAMBOS.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")
if not redis:get(RAMBOS.."lock_STCK"..msg.chat_id_) then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"منور انت ، 😙","اه باع وجهك شكد نضيف 😅","هذا منو ، 😏","تسمحلي ابوسك ☹️😹","مليان ضحك مليان 😹❤️","تف على هذا  ويهك 💦😹","اذا حاته ممكن الرقم 😆😹","تدري صار "..(redis:get(RAMBOS.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")).." ملصق داز  شهالتبذير 🤔😹","كافي ملصقات مشايف 😫"}
send(msg.chat_id_, msg.id_, 1,""..RAMBOS[math.random(#RAMBOS)].."", 1, 'md')
end
end
...
redis:incr(RAMBOS.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Voice ] <<\027[00m")
msg_type = "MSG:Voice"
end
if msg.content_.ID == "MessageVideo" then
redis:incr(RAMBOS.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")
print("\027[" .. color.black[1] .. ";" .. color.yellow[2] .. "m>> [ Video ] <<\027[00m")
msg_type = "MSG:Video"
end
if msg.content_.ID == "MessageAnimation" then
redis:incr(RAMBOS.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
redis:incr(RAMBOS.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")
...
local content_text = redis:get(RAMBOS..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_text == 'save_repgp' then
redis:del(RAMBOS..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
local content_text = redis:get(RAMBOS..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
if msg.content_.video_ then
redis:set(RAMBOS..'video_repgp'..content_text..''..msg.chat_id_..'', msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(RAMBOS..'stecker_repgp'..content_text..''..msg.chat_id_..'', msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(RAMBOS..'voice_repgp'..content_text..''..msg.chat_id_..'', msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(RAMBOS..'gif_repgp'..content_text..''..msg.chat_id_..'', msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(RAMBOS..'text_repgp'..content_text..''..msg.chat_id_..'', msg.content_.text_)
end
redis:sadd('rep_owner'..msg.chat_id_..'',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(RAMBOS..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end end
if msg.content_.text_ and not redis:get(RAMBOS..'lock_reeeep'..msg.chat_id_) then
if redis:get(RAMBOS..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(RAMBOS..'video_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(RAMBOS..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(RAMBOS..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if  redis:get(RAMBOS..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(RAMBOS..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(RAMBOS..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(RAMBOS..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..''))
end
if redis:get(RAMBOS..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') then
send(msg.chat_id_, msg.id_, 1, redis:get(RAMBOS..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'') ,  1, 'md')
end
end
---------------------------------in all gps---------------------------------------------------
text = msg.content_.text_
if msg.content_.text_  or msg.content_.video_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.animation_ then
local content_text = redis:get(RAMBOS.."add:repallt"..msg.sender_user_id_)
if content_text == 'save_rep' then
redis:del(RAMBOS.."add:repallt"..msg.sender_user_id_)
local content_text = redis:get(RAMBOS.."addreply2:"..msg.sender_user_id_)
if msg.content_.video_ then
redis:set(RAMBOS.."video_repall"..content_text, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.sticker_ then
redis:set(RAMBOS.."stecker_repall"..content_text, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
redis:set(RAMBOS.."voice_repall"..content_text, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.animation_ then
redis:set(RAMBOS.."gif_repall"..content_text, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
redis:set(RAMBOS.."text_repall"..content_text, msg.content_.text_)
end
redis:sadd('rep_sudo',content_text)
send(msg.chat_id_, msg.id_, 1, "📌┊ تم حفظ الرد بنجاح\n", 1, 'md')
redis:del(RAMBOS.."addreply2:"..msg.sender_user_id_)
return false end end
if msg.content_.text_ and not redis:get(RAMBOS..'lock_reeeep'..msg.chat_id_) then
if redis:get(RAMBOS.."video_repall"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(RAMBOS.."video_repall"..msg.content_.text_))
end
if redis:get(RAMBOS.."voice_repall"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(RAMBOS.."voice_repall"..msg.content_.text_))
end
if  redis:get(RAMBOS.."gif_repall"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, redis:get(RAMBOS.."gif_repall"..msg.content_.text_))
end
if redis:get(RAMBOS.."stecker_repall"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, redis:get(RAMBOS.."stecker_repall"..msg.content_.text_))
end
if redis:get(RAMBOS.."text_repall"..msg.content_.text_) then
send(msg.chat_id_, msg.id_, 1, redis:get(RAMBOS.."text_repall"..msg.content_.text_) ,  1, "md")
end
end
...
if not redis:get(RAMBOS..flmax) then
floodMax = 5
else
floodMax = tonumber(redis:get(RAMBOS..flmax))
end
local pm = "flood:" .. msg.sender_user_id_ .. ":" .. msg.chat_id_ .. ":msgs"
if not redis:get(RAMBOS..pm) then
msgs = 0
else
msgs = tonumber(redis:get(RAMBOS..pm))
end
local TIME_CHECK = 2
local TIME_CHECK_PV = 2
local hashflood = "anti-flood:" .. msg.chat_id_
if msgs > floodMax - 1 then
if redis:get(RAMBOS.."floodstatus" .. msg.chat_id_) == "Kicked" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
elseif redis:get(RAMBOS.."floodstatus" .. msg.chat_id_) == "DelMsg" then
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
else
del_all_msgs(msg.chat_id_, msg.sender_user_id_)
end
...
if not redis:get(RAMBOS..pmonpv) then
msgsonpv = 0
else
msgsonpv = tonumber(redis:get(RAMBOS..pmonpv))
end
if msgsonpv > 12 then
blockUser(msg.sender_user_id_)
end
local idmem = tostring(msg.chat_id_)
if idmem:match("^(%d+)") and not is_admin(msg.sender_user_id_) and not redis:get(RAMBOS.."Filtering:" .. msg.sender_user_id_) then
redis:setex(RAMBOS..pmonpv, TIME_CHECK_PV, msgsonpv + 1)
end
...
if redis:get(RAMBOS.."keed_link"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
local chat = msg.chat_id_
delete_msg(chat, msgs)
HTTPS.request("https://api.telegram.org/bot" .. RAMBOS_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(RAMBOS..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(RAMBOS.."keed_fosh"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."keed_user"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(RAMBOS.."keed_photo" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."keed_text"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if text and text:match("(.*)(#)(.*)")  then
if redis:get(RAMBOS.."keed_hashtag"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."keed_fwd"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageSticker" then
if redis:get(RAMBOS.."keed_stecker"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAudio" then
if redis:get(RAMBOS.."keed_audio"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVoice" then
if redis:get(RAMBOS.."keed_voice"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageVideo" then
if redis:get(RAMBOS.."keed_video"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageAnimation" then
if redis:get(RAMBOS.."keed_gif"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageContact" then
if redis:get(RAMBOS.."keed_contect"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."keed_arbic"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessageDocument" then
if redis:get(RAMBOS.."keed_Document"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."keed_english"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."keed_markdon"..msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(RAMBOS.."bot:muted:Time" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and redis:sismember(RAMBOS.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_) then
redis:srem(RAMBOS.."bot:muted:" .. msg.chat_id_, msg.sender_user_id_)
end
if is_gbanned(msg.sender_user_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
return
end
if redis:get(RAMBOS.."bot:muteall:Time" .. msg.chat_id_) then
local start_ = redis:get(RAMBOS.."bot:muteall:start" .. msg.chat_id_)
local start = start_:gsub(":", "")
local stop_ = redis:get(RAMBOS.."bot:muteall:stop" .. msg.chat_id_)
local stop = stop_:gsub(":", "")
local SVTime = os.date("%R")
local Time = SVTime:gsub(":", "")
if tonumber(Time) >= tonumber(start) and not redis:get(RAMBOS.."bot:muteall:Run" .. msg.chat_id_) then
local g = os.time()
redis:set(RAMBOS.."bot:muteall:start_Unix" .. msg.chat_id_, g)
...
redis:set(RAMBOS.."bot:muteall:stop_Unix" .. msg.chat_id_, unix)
redis:set(RAMBOS.."bot:muteall:Run" .. msg.chat_id_, true)
end
end
if redis:get(RAMBOS.."bot:muteall:Time" .. msg.chat_id_) and redis:get(RAMBOS.."bot:muteall:Run" .. msg.chat_id_) then
local SR = redis:get(RAMBOS.."bot:muteall:start_Unix" .. msg.chat_id_) or 0
local SP = redis:get(RAMBOS.."bot:muteall:stop_Unix" .. msg.chat_id_) or 0
...
redis:del(RAMBOS.."bot:muteall:Run" .. msg.chat_id_)
end
end
---------------------pinnedmsg----------------------------------------------------------
if msg.content_.ID == "MessagePinMessage" and not msg.sender_user_id_ == our_id and not is_owner(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOS.."pinnedmsg" .. msg.chat_id_) and redis:get(RAMBOS.."bot:pin:mute" .. msg.chat_id_) then
unpinmsg(msg.chat_id_)
local pin_id = redis:get(RAMBOS.."pinnedmsg" .. msg.chat_id_)
pinmsg(msg.chat_id_, pin_id, 0)
end
if not redis:get(RAMBOS.."Resetdatapost" .. msg.chat_id_) then
redis:del(RAMBOS.."Gp:Post" .. msg.chat_id_)
redis:setex(RAMBOS.."Resetdatapost" .. msg.chat_id_, 12 * hour, true)
end
----------------------com viewget -----------------------------------------------------
if redis:get(RAMBOS.."bot:viewget" .. msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, "✯↓ حدث خطا حاول مره اخرى", 1, "md")
redis:del(RAMBOS.."bot:viewget" .. msg.sender_user_id_)
else
send(msg.chat_id_, msg.id_, 1, "📛↓ عـــدد المشاهـــدات \n: " .. msg.views_ .. "", 1, "md")
redis:del(RAMBOS.."bot:viewget" .. msg.sender_user_id_)
end end
if redis:get(RAMBOS.."bot:viewmsg") == "On" then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_) then
...
redis:del(RAMBOS..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
setphoto(msg.chat_id_, photo_id)
end
------------------------------------------------------------------------------------------------
if redis:get(RAMBOS.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local gps = redis:scard(RAMBOS.."bot:groups") or 0
local gpss = redis:smembers(RAMBOS.."bot:groups") or 0
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:del(RAMBOS.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
if redis:get(RAMBOS.."anti-flood:" .. msg.chat_id_) then
redis:setex(RAMBOS..pm, TIME_CHECK, msgs + 1)
end
--«««««««««««««««««««««««« Developer By Karrar RAMBOS »»»»»»»»»»»»»»»»»»»»»»»»»»»--
if msg.content_.caption_ then
...
if redis:get(RAMBOS.."bot:duplipost:mute" .. msg.chat_id_) then
if redis:sismember(RAMBOS.."Gp:Post" .. msg.chat_id_, post) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:sadd(RAMBOS.."Gp:Post" .. msg.chat_id_, post)
end
end
if redis:get(RAMBOS.."post:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.forward_info_ and redis:get(RAMBOS.."bot:forward:mute" .. msg.chat_id_) and (msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost") then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.ID == "MessagePhoto" then
if redis:get(RAMBOS.."bot:photo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:strict" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
end
if msg.content_.caption_:match("@") then
if redis:get(RAMBOS.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityMention" and redis:get(RAMBOS.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("#") then
if redis:get(RAMBOS.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and msg.content_.entities_[0].ID == "MessageEntityHashtag" and redis:get(RAMBOS.."bot:hashtag:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
elseif msg.content_.entities_ and msg.content_.entities_[0] and (msg.content_.entities_[0].ID == "MessageEntityTextUrl" or msg.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(RAMBOS.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.web_page_ and redis:get(RAMBOS.."bot:webpage:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\128-\191]") and redis:get(RAMBOS.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if (msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]")) and redis:get(RAMBOS.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."markdown:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."Game:lock" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOS.."tags:lock" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:document:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_:match("[\216-\219][\121-\191]") and redis:get(RAMBOS.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:inline:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."clerk") == "On" or is_admin(msg.sender_user_id_) then
...
if redis:get(RAMBOS.."bot:sticker:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:tgservice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not is_vipmem(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOS.."bot:member:lock" .. msg.chat_id_) then
chat_kick(msg.chat_id_, msg.sender_user_id_)
end
...
for i = 0, #list do
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(RAMBOS.."bot:bots:mute" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(RAMBOS.."bot:botskick" .. msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
chat_kick(msg.chat_id_, msg.sender_user_id_)
local user_info_ = redis:get(RAMBOS..'user:Name' .. msg.sender_user_id_)
local UserRAMBOS = user_info_
if user_info_ then
send(msg.chat_id_, msg.id_, 1, "👨‍✈️┊ العضو » (["..UserRAMBOS.."])\n🚫┊ الايدي » (*"..msg.sender_user_id_.."*)\n🗯┊ قام بأضافه بوت في المجموعه\n📌┊ تم طرد البوت مع العضو \n✓", 1, "md")
end 
end
if list[i].type_.ID == "UserTypeBot" and not is_momod(list[i].id_, msg.chat_id_) and redis:get(RAMBOS.."keed_bots"..msg.chat_id_) then
chat_kick(msg.chat_id_, list[i].id_)
redis:sadd(RAMBOS..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
HTTPS.request("https://api.telegram.org/bot" .. RAMBOS_TOKEN .. "/restrictChatMember?chat_id=" .. msg.chat_id_ .. "&user_id=" .. msg.sender_user_id_ .. "&can_send_messages=false&can_send_media_messages=false&can_send_other_messages=false&can_add_web_page_previews=false")
redis:sadd(RAMBOS..'bot:keed:'..msg.chat_id_, msg.sender_user_id_)
...
if redis:get(RAMBOS.."bot:member:lock" .. msg.chat_id_) and not is_vipmem(msg.content_.members_[0].id_, msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
...
if redis:get(RAMBOS.."bot:contact:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:music:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:voice:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:location:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:video:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:selfvideo:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:gifs:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.caption_ then
check_filter_words(msg, msg.content_.caption_)
if redis:get(RAMBOS.."bot:links:mute" .. msg.chat_id_) and (msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://")) then
local id = msg.id_
local msgs = {
[0] = id
}
...
redis:setex(RAMBOS.."bot:editid" .. msg.id_, day, msg.content_.text_)
if redis:get(RAMBOS.."anti-flood:" .. msg.chat_id_) and not is_vipmem(msg.sender_user_id_, msg.chat_id_) then
redis:setex(RAMBOS..pm, TIME_CHECK, msgs + 1)
end
---------------------rem cam filters---------------------------------------------------------------------
if redis:get(RAMBOS.."Filtering:" .. msg.sender_user_id_) then
local chat = redis:get(RAMBOS.."Filtering:" .. msg.sender_user_id_)
...
---------------save name bot-----------------------------
if redis:get(RAMBOS..'botts:namess'..msg.sender_user_id_) then
redis:del(RAMBOS..'botts:namess'..msg.sender_user_id_)
local NAME_BOT = msg.content_.text_:match("(.*)")
redis:set(RAMBOS..'RAMBOSnams',NAME_BOT)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم وضع اسم البوت 🍃",1, 'html')
return false
end
------------------------save cam link-----------------------------------
if redis:get(RAMBOS.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
local glink = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
local hash = "bot:group:link" .. msg.chat_id_
redis:set(RAMBOS..hash, glink)
send(msg.chat_id_, msg.id_, 1, "🌀┊ تـــم  حفــــظ الرابط 🎈", 1, "md")
redis:del(RAMBOS.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
--------------------------------------------------------------------------
if redis:get(RAMBOS.."gettextsec" .. msg.sender_user_id_) then
local clerktext = msg.content_.text_
redis:set(RAMBOS.."textsec", clerktext)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تم حفظ الكليشه 🎈", 1, "md")
redis:del(RAMBOS.."gettextsec" .. msg.sender_user_id_)
end
---------------------save com rules-----------------------------------------------------------------------------
if redis:get(RAMBOS.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local rules = msg.content_.text_
redis:set(RAMBOS.."bot:rules" .. msg.chat_id_, rules)
send(msg.chat_id_, msg.id_, 1, "🌀┊  تـــم حفــــظ القوانين 🎋",  1, "md")
redis:del(RAMBOS.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
end
-----------ch join------------------------------------------------------
if redis:get(RAMBOS..'rmch'..msg.sender_user_id_) then
redis:del(RAMBOS..'rmch'..msg.sender_user_id_)
local url , res = https.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/getChatAdministrators?chat_id='..msg.content_.text_..'')
local data = RMJS:decode(url)
if res == 400 then
...
return false 
elseif data.description == "Bad Request: chat not found" then -- التحقق من المعرف (@rmchh)
...
--------------------rem  cam broadcast2--------------------------------------------------------------------------
if redis:get(RAMBOS.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
if text:match("^الغاء$") then
send(msg.chat_id_, msg.id_, 1, "🔰- تم الغاء الامر بنجاح🎈 ", 1, "md")
redis:del(RAMBOS.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
else
local gps = redis:scard(RAMBOS.."bot:groups") or 0
local gpss = redis:smembers(RAMBOS.."bot:groups") or 0
local msgs = {
...
if redis:get(RAMBOS.."bot:joinbylink:" .. msg.sender_user_id_) and (msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")) then
else
end
--------------------set cam sudo  ------------------------------------------
if redis:get(RAMBOS.."bot:RAMBOS_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
redis:del(RAMBOS.."bot:RAMBOS_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
local RAMBOS_dev = msg.content_.text_:match("(.*)")
redis:set(RAMBOS.."RAMBOS_dev", RAMBOS_dev)
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(RAMBOS.."bot:links:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:text:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if msg.content_.text_:match("[\216-\219][\121-\191]") and redis:get(RAMBOS.."bot:arabic:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if not redis:get(RAMBOS..hash) then
sens = 400
else
sens = tonumber(redis:get(RAMBOS..hash))
end
if redis:get(RAMBOS.."bot:spam:mute" .. msg.chat_id_) and string.len(msg.content_.text_) > sens or ctrl_chars > sens or real_digits > sens then
delete_msg(chat, msgs)
print_del_msg("Deleted Because [Lock] [Spam] ")
end
end
if (msg.content_.text_:match("[A-Z]") or msg.content_.text_:match("[a-z]")) and redis:get(RAMBOS.."bot:english:mute" .. msg.chat_id_) then
local id = msg.id_
local msgs = {
[0] = id
}
...
if redis:get(RAMBOS.."bot:cmds" .. msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
print("\01[" .. color.white[1] .. ";" .. color.magenta[2] .. "m>>> Lock Cmd Is Active In This Group <<<\01[00m")
return false
end
end
if is_owner(msg.sender_user_id_, msg.chat_id_) and not rm_JoinCh(msg) or is_monshi(msg.sender_user_id_, msg.chat_id_) and not rm_JoinCh(msg) or is_sudo(msg) and not rm_JoinCh(msg) then
return false
end
-------------------------------leave groups----------------------------------------------------------------------
if is_sudo(msg) and idf:match("-100(%d+)") and (text:match('^'..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..' غادر$')) then
...
redis:set(RAMBOS.."RAMBOS_O" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(RAMBOS.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_RAMBOS = tonumber(redis:get(RAMBOS.."RAMBOS_O" .. msg.chat_id_)) 
if msgs == get_RAMBOS and not redis:get(RAMBOS.."lock_addd"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ هذا العضو اثبت تفاعله\n🎟┊تم رفعه ادمن في المجموعه\n✓",  1, "md")
redis:sadd(RAMBOS..'bot:momod:'..msg.chat_id_, msg.sender_user_id_)
end
--------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "⚜️┊ تم حفظ العدد *"..uuuu[2].."*\n💬┊ سيتم رفع العضو مميز\n🚫┊اذا اثبت تفاعله\n✓",1, 'md')
redis:set(RAMBOS.."RAMBOS_OO" .. msg.chat_id_, uuuu[2])
end
local msgs = tonumber(redis:get(RAMBOS.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local get_RAMBOS = tonumber(redis:get(RAMBOS.."RAMBOS_OO" .. msg.chat_id_)) 
if msgs == get_RAMBOS and not redis:get(RAMBOS.."lock_adddvip"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "📌┊ هذا العضو اثبت تفاعله\n🌀┊تم رفعه عضو مميز في المجموعه\n✓",  1, "md")
redis:sadd(RAMBOS..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_)
end
------------------------------ADD vipmems BY Reply------------------------------------------------------------------
if text:match('^رفع مميز عام$') and is_RM(msg) and msg.reply_to_message_id_ ~= 0  then
function promote_by_reply(extra, result, success)
local user_info_ = redis:get(RAMBOS..'user:Name' .. result.sender_user_id_)
local UserRAMBOS = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(RAMBOS..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(RAMBOS..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم رفعه مميز عام\n✓', 1, 'md')
end end end
getMessage(msg.chat_id_, msg.reply_to_message_id_,promote_by_reply)
end
---------------------------ADD vipmems BY USER--------------------------------------------------------------------
if text:match('^رفع مميز عام @(%S+)$') and is_RM(msg) then
local ap = {string.match(text, '^(رفع مميز عام) @(%S+)$')}
function promote_by_username(extra, result, success)
local hash = 'bot:vipmems:'
if result.id_ then
if redis:sismember(RAMBOS..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(RAMBOS..hash, result.id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
resolve_username(ap[2],promote_by_username)
end
---------------------------ADD vipmems BY ID--------------------------------------------------------------------
if text:match('^رفع مميز عام (%d+)$') and is_RM(msg) then
local ap = {string.match(text, '^(رفع مميز عام) (%d+)$')}
local user_info_ = redis:get(RAMBOS..'user:Name' .. ap[2])
local UserRAMBOS = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if redis:sismember(RAMBOS..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ مرفوع مميز عام سابقا\n✓‏', 1, 'md')
else
redis:sadd(RAMBOS..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم رفعه مميز عام\n✓‏', 1, 'md')
end end end
----------------------DEL vipmems BY REPLY-------------------------------------------------------------------------
if text:match('^تنزيل مميز عام$') and is_RM(msg) and msg.reply_to_message_id_ ~= 0 then
function demote_by_reply(extra, result, success)
local user_info_ = redis:get(RAMBOS..'user:Name' .. result.sender_user_id_)
local UserRAMBOS = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(RAMBOS..hash, result.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(RAMBOS..hash, result.sender_user_id_)
...
------------------------DEL vipmems BY USER-----------------------------------------------------------------------
if text:match('^تنزيل مميز عام @(%S+)$') and is_RM(msg) then
local ap = {string.match(text, '^(تنزيل مميز عام) @(%S+)$')}
function demote_by_username(extra, result, success)
local hash = 'bot:vipmems:'
if result.id_ then
if not redis:sismember(RAMBOS..hash, result.id_) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تنزيله مميز عام سابقا\n‏', 1, 'md')
else
redis:srem(RAMBOS..hash, result.id_)
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تنزيله من مميزين العام \n✓‏', 1, 'md')
end end end
resolve_username(ap[2],demote_by_username)
end
--------------------------DEL vipmems BY ID---------------------------------------------------------------------
if text:match('^تنزيل مميز عام (%d+)$') and is_RM(msg) then
local ap = {string.match(text, '^(تنزيل مميز عام) (%d+)$')}
local user_info_ = redis:get(RAMBOS..'user:Name' .. ap[2])
local UserRAMBOS = user_info_
if user_info_ then
local hash = 'bot:vipmems:'
if not redis:sismember(RAMBOS..hash, ap[2]) then
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم تنزيله مميز عتم سابقا\n‏', 1, 'md')
else
redis:srem(RAMBOS..hash, ap[2])
send(msg.chat_id_, msg.id_, 1,'👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم حذفه من مميزين العام \n✓‏', 1, 'md')
end end
end
-----------------------------promote_by_reply-------------------------------------------------------
...
function promote_by_username(extra, result, success)
if result.id_ then
if redis:sismember(RAMBOS..'bot:vipmem:'..msg.chat_id_, result.id_) then
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(RAMBOS..'bot:vipmem:'..msg.chat_id_, result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end
...
local UserRAMBOS = user_info_
if user_info_ then
if redis:sismember(RAMBOS..'bot:vipmem:'..msg.chat_id_, ap[2]) then
texts = '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ مرفوع مميز سابقا\n✓'
else
redis:sadd(RAMBOS..'bot:vipmem:'..msg.chat_id_, ap[2])
texts = '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم رفعه عضو مميز\n✓'
end end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
--------------------------gban_by_reply---------------------------------------------------------------------
if text:match('^حظر عام$') and is_RM(msg) and msg.reply_to_message_id_ then
function ban_by_reply(extra, result, success)
local hash = 'bot:gban:'
if is_momod(result.sender_user_id_, result.chat_id_) then
send(msg.chat_id_, msg.id_, 1, '🌀┊ عذراً لا استطيع (حظر،طرد،كتم)المدراء والادمنيه ❗️', 1, 'md')
...
--------------------------ban_by_username---------------------------------------------------------------------
if text:match('^حظر عام @(%S+)$') and is_RM(msg) then
...
------------------------ban_by_id-----------------------------------------------------------------------
if text:match('^حظر عام (%d+)$') and is_RM(msg) then
...
-----------------------ungban_by_reply------------------------------------------------------------------------
if text:match('^الغاء العام$') and is_RM(msg) and msg.reply_to_message_id_ then
function ban_by_reply(extra, result, success)
local hash = 'bot:gban:'
local user_info_ = redis:get(RAMBOS..'user:Name' .. result.sender_user_id_)
local UserRAMBOS = user_info_
...
--------------------------ban_by_username---------------------------------------------------------------------
if text:match('^الغاء العام @(%S+)$') and is_RM(msg) then
...
------------------------ban_by_id-----------------------------------------------------------------------
if text:match('^الغاء العام (%d+)$') and is_RM(msg) then
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'')
redis:sadd(RAMBOS..hash, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'')
redis:sadd(RAMBOS..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'')
redis:sadd(RAMBOS..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOS..hash, result.sender_user_id_)
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOS..hash, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
HTTPS.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOS..hash, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم الغاء تقييده بنجاح \n✓‏', 1, 'md')
end
end
...
if result.id_ then
redis:sadd(RAMBOS..'bot:admins:', result.id_)
texts = '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم اضافه ادمن في البوت \n✓‏'
else
...
redis:del(RAMBOS..'bot:allmsgs')
send(msg.chat_id_, msg.id_, 1, '🌀┊ تم حذف رسائل البوت في المجموعه', 1, 'md')
end
-------------ADD RAMBOS_SUDO----------------------------------------------------------------------------------
if is_RM(msg) and text:match('^رفع مطور$') and msg.reply_to_message_id_ then
...
-------------ADD RAMBOS_SUDO----------------------------------------------------------------------------------
if text:match('^رفع مطور @(%S+)$') and is_RM(msg) then
...
-------------ADD RAMBOS_SUDO----------------------------------------------------------------------------------
if text:match('^رفع مطور (%d+)$') and is_RM(msg) then
...
--------------REM RAMBOS_SUDO---------------------------------------------------------------------------------
if is_RM(msg) and text:match('^تنزيل مطور$') and msg.reply_to_message_id_ then
...
--------------REM RAMBOS_SUDO---------------------------------------------------------------------------------
if text:match('^تنزيل مطور @(%S+)$') and is_RM(msg) then
...
--------------REM RAMBOS_SUDO---------------------------------------------------------------------------------
if text:match('^تنزيل مطور (%d+)$') and is_RM(msg) then
...
local UserRAMBOS = user_info_
if user_info_ then
local k1 = not redis:sismember(RAMBOS..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(RAMBOS..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(RAMBOS..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(RAMBOS..'bot:owners:'..msg.chat_id_, result.sender_user_id_)
redis:srem(RAMBOS..'bot:momod:'..msg.chat_id_, result.sender_user_id_)
redis:srem(RAMBOS..'bot:vipmem:'..msg.chat_id_, result.sender_user_id_)
...
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(RAMBOS..'bot:owners:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(RAMBOS..'bot:momod:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(RAMBOS..'bot:vipmem:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(RAMBOS..'bot:owners:'..msg.chat_id_, result.id_)
redis:srem(RAMBOS..'bot:momod:'..msg.chat_id_, result.id_)
redis:srem(RAMBOS..'bot:vipmem:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم حذف كل الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
local UserRAMBOS = user_info_
if user_info_ then
local k1 = not redis:sismember(RAMBOS..'bot:owners:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(RAMBOS..'bot:momod:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(RAMBOS..'bot:vipmem:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ لا توجد لديه رتبه\n✓‏', 1, 'md')
else
redis:srem(RAMBOS..'bot:owners:'..msg.chat_id_, ap[2])
redis:srem(RAMBOS..'bot:momod:'..msg.chat_id_, ap[2])
redis:srem(RAMBOS..'bot:vipmem:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم حذف كل الرتب عنه \n✓‏', 1, 'md')
end
end
end
...
local UserRAMBOS = user_info_
if user_info_ then
local k1 = not redis:sismember(RAMBOS..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
local k2 = not redis:sismember(RAMBOS..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
local k3 = not redis:sismember(RAMBOS..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ لا توجد لديه قيـــــــود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.sender_user_id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOS..'bot:keed:'..msg.chat_id_, result.sender_user_id_)
redis:srem(RAMBOS..'bot:muted:'..msg.chat_id_, result.sender_user_id_)
redis:srem(RAMBOS..'bot:banned:'..msg.chat_id_, result.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
function delallrtb(extra, result, success)
if result.id_ then
local k1 = not redis:sismember(RAMBOS..'bot:keed:'..msg.chat_id_, result.id_)
local k2 = not redis:sismember(RAMBOS..'bot:muted:'..msg.chat_id_, result.id_)
local k3 = not redis:sismember(RAMBOS..'bot:banned:'..msg.chat_id_, result.id_)
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ لا توجد لديه قيـــــــود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..result.id_..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOS..'bot:keed:'..msg.chat_id_, result.id_)
redis:srem(RAMBOS..'bot:muted:'..msg.chat_id_, result.id_)
redis:srem(RAMBOS..'bot:banned:'..msg.chat_id_, result.id_)
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » ([@'..ap[2]..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
local UserRAMBOS = user_info_
if user_info_ then
local k1 = not redis:sismember(RAMBOS..'bot:keed:'..msg.chat_id_, ap[2])
local k2 = not redis:sismember(RAMBOS..'bot:muted:'..msg.chat_id_, ap[2])
local k3 = not redis:sismember(RAMBOS..'bot:banned:'..msg.chat_id_, ap[2])
if k1 and k2 and k3 then
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ لا توجد لديه قيـــــــود\n✓‏', 1, 'md')
else
HTTPS.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/restrictChatMember?chat_id=' ..msg.chat_id_.. '&user_id=' ..ap[2]..'&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True')
redis:srem(RAMBOS..'bot:keed:'..msg.chat_id_, ap[2])
redis:srem(RAMBOS..'bot:muted:'..msg.chat_id_, ap[2])
redis:srem(RAMBOS..'bot:banned:'..msg.chat_id_, ap[2])
send(msg.chat_id_, msg.id_, 1, '👨‍✈️┊ العضو » (['..UserRAMBOS..'])\n⚠️┊ تم رفع القيود عنه \n✓‏', 1, 'md')
end
end
end
...
if redis:get(RAMBOS.."lock_GEM"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ اللعبه معطله\n ‏ ", 1, "md")
return false end
local user_info_ = redis:get(RAMBOS.."user:Name" .. msg.sender_user_id_)
local UserRAMBOS = user_info_
if user_info_ then
redis:set(RAMBOS..'kk1'..msg.sender_user_id_..''..msg.chat_id_..'','kk')
send(msg.chat_id_, 0, 1, '👨‍✈️» اهلا ['..UserRAMBOS..'] \n™️» في لعبه السرعه\n⚠️» ارسل ( بدء اللعبه ) للعب\n✓',1, 'md')
return false end end
if text == 'بدء اللعبه'  and is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOS..'kk1'..msg.sender_user_id_..''..msg.chat_id_..'') then
local keep1 = {'س م ى و','ى-س-م-و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep1[math.random(#keep1)]..')*',1, 'md')
redis:set(RAMBOS..'kk11'..msg.chat_id_..'','kkk')
return false end
if text then
local RAMBOS1 = redis:get(RAMBOS..'kk11'..msg.chat_id_..'')
if RAMBOS1 == 'kkk' then
if text == 'موسى' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'kk11'..msg.chat_id_..'')
sleep(1.5)
local ooo = {'ا-ل-ي-ي-ب','ي ي ا ل ب'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..ooo[math.random(#ooo)]..')*',1, 'md')
redis:set(RAMBOS..'kk111'..msg.chat_id_..'','kkkk')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'kk111'..msg.chat_id_..'')
if RAMBOS1 == 'kkkk' then
if text == 'ليبيا' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'kk111'..msg.chat_id_..'')
sleep(1.5) 
local keep = {'ر ط ا ي ه','ا-ي-ط-ر-ه'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep[math.random(#keep)]..')*',1, 'md')
redis:set(RAMBOS..'kk1111'..msg.chat_id_..'','mm')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'kk1111'..msg.chat_id_..'')
if RAMBOS1 == 'mm' then
if text == 'طياره' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'kk1111'..msg.chat_id_..'')
sleep(1.5)
local keep77 = {'😛😛😛😝😛😛😛','😜😝😜😜😜😜😜'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep77[math.random(#keep77)]..')*',1, 'md')
redis:set(RAMBOS..'w1'..msg.chat_id_..'','q1')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w1'..msg.chat_id_..'')
if RAMBOS1 == 'q1' then
if text == '😝' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w1'..msg.chat_id_..'')
sleep(1.5)
local keep2 = {'ش-ر-ط-ي','ش,ط,ر,ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep2[math.random(#keep2)]..')*',1, 'md')
redis:set(RAMBOS..'w2'..msg.chat_id_..'','q2')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w2'..msg.chat_id_..'')
if RAMBOS1 == 'q2' then
if text == 'شرطي' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w2'..msg.chat_id_..'')
sleep(1.5)
local keep3 = {'😃😃😄😃😃😃'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep3[math.random(#keep3)]..')*',1, 'md')
redis:set(RAMBOS..'w9'..msg.chat_id_..'','q9')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w9'..msg.chat_id_..'')
if RAMBOS1 == 'q9' then
if text == '😄' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w9'..msg.chat_id_..'')
sleep(1.5)
local keep4 = {'ر ك و ا ي','ر ا ي ك و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep4[math.random(#keep4)]..')*',1, 'md')
redis:set(RAMBOS..'w4'..msg.chat_id_..'','q4')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w4'..msg.chat_id_..'')
if RAMBOS1 == 'q4' then
if text == 'كوريا' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w4'..msg.chat_id_..'')
sleep(1.5)
local keep5 = {'ك - م - ا - ل','ل ك ا م'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep5[math.random(#keep5)]..')*',1, 'md')
redis:set(RAMBOS..'w5'..msg.chat_id_..'','q5')
return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w5'..msg.chat_id_..'')
if RAMBOS1 == 'q5' then
if text == 'ملاك' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w5'..msg.chat_id_..'')
sleep(1.5)
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n(ك , ش , ب , ا)',1, 'md')
redis:set(RAMBOS..'w6'..msg.chat_id_..'','q6')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w6'..msg.chat_id_..'')
if RAMBOS1 == 'q6' then
if text == 'شباك' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w6'..msg.chat_id_..'')
sleep(1.5)
local keep6 = {'ل-ج-م-ي','ل ي م ج'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep6[math.random(#keep6)]..')*',1, 'md')
redis:set(RAMBOS..'w7'..msg.chat_id_..'','q7')
return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w7'..msg.chat_id_..'')
if RAMBOS1 == 'q7' then
if text == 'جميل' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w7'..msg.chat_id_..'')
sleep(1.5)
local keep7 = {'و ك ه ه','ه ك ه و'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep7[math.random(#keep7)]..')*',1, 'md')
redis:set(RAMBOS..'w8'..msg.chat_id_..'','q8')
return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w8'..msg.chat_id_..'')
if RAMBOS1 == 'q8' then
if text == 'كهوه' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w8'..msg.chat_id_..'')
sleep(1.5)
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n( ن ،ص، و، ك، د)',1, 'md')
redis:set(RAMBOS..'w9o'..msg.chat_id_..'','q9o')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w9o'..msg.chat_id_..'')
if RAMBOS1 == 'q9o' then
if text == 'صندوك' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w9o'..msg.chat_id_..'')
sleep(1.5)
local keep8 = {'ض ر م ي','م ض ر ي'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep8[math.random(#keep8)]..')*',1, 'md')
redis:set(RAMBOS..'w00'..msg.chat_id_..'','q00')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'w00'..msg.chat_id_..'')
if RAMBOS1 == 'q00' then
if text == 'مريض' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'w00'..msg.chat_id_..'')
sleep(1.5)
local keep9 = {'ي- ط -ر -ق -ن -ب -ا',' ب ي ق ا ط ر ا ن','ب,ي,ا,,ق,ر,ن,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keep9[math.random(#keep9)]..')*',1, 'md')
redis:set(RAMBOS..'a15'..msg.chat_id_..'','s15')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'a15'..msg.chat_id_..'')
if RAMBOS1 == 's15' then
if text == 'قرنابيط' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'a15'..msg.chat_id_..'')
sleep(1.5)
local keep00 = {'😔😔😔😔😔😔😔☺️😔😔😔😔😔'}
send(msg.chat_id_, 0, 1, '•ارسل الاسمايل المختلف \n*('..keep00[math.random(#keep00)]..')*',1, 'md')
redis:set(RAMBOS..'a26'..msg.chat_id_..'','s26')
return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'a26'..msg.chat_id_..'')
if RAMBOS1 == 's26' then
if text == '☺️' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'a26'..msg.chat_id_..'')
sleep(1.5)
local keepp = {'ط - م - ي','ي ,م ,ط'}
send(msg.chat_id_, 0, 1, '• رتب الكلمه  التاليه \n*('..keepp[math.random(#keepp)]..')*',1, 'md')
redis:set(RAMBOS..'a99'..msg.chat_id_..'','s99')
 return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'a99'..msg.chat_id_..'')
if RAMBOS1 == 's99' then
if text == 'مطي' then
redis:incr(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
send(msg.chat_id_, msg.id_, 1, '• اجابتك صحيحه 👏🏻 ',1, 'md')
redis:del(RAMBOS..'a99'..msg.chat_id_..'')
return false end end end
-----------------------------------------------------------------------------
if text == 'نقاطي' then
send(msg.chat_id_, msg.id_, 1, '💬┊ عدد نقاطك » *('..tonumber(redis:get(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )..')*',1, 'md')
return false end
if text == 'بيع نقاطي' then
local user_info_ = redis:get(RAMBOS..'user:Name' .. msg.sender_user_id_)
local UserRAMBOS = user_info_
if user_info_ then
local numkep = tonumber(redis:get(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
if numkep == 0 then
...
redis:set(RAMBOS..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'','karrar2')
send(msg.chat_id_, msg.id_, 1, keep,1, 'md')
return false end end end
if text then
local RAMBOS1 = redis:get(RAMBOS..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
if RAMBOS1 == 'karrar2' then
if text == '1' then
if redis:sismember(RAMBOS..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_) then
send(msg.chat_id_, msg.id_, 1,'⚠️┊ عذرا انت مرفوع مميز سابقا\n✓', 1, 'md')
redis:del(RAMBOS..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
else
local numkep = tonumber(redis:get(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
if numkep < tonumber(redis:get(RAMBOS.."RAMBOS_OO0" .. msg.chat_id_) or 100 )  then
send(msg.chat_id_, msg.id_, 1, '®️┊عذرا نقاطك اقل من '..tonumber(redis:get(RAMBOS.."RAMBOS_OO0" .. msg.chat_id_) or 100 )..'\n💬┊لا تستطيع تحويلهم لعضو مميز\n✓',1, 'md')
redis:del(RAMBOS..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
return false end
if numkep > tonumber(redis:get(RAMBOS.."RAMBOS_OO0" .. msg.chat_id_) or 100 )  then
send(msg.chat_id_, msg.id_, 1, '🎰┊مبروك عزيزي\n🎧┊تم رفعك عضو مميز\n ✓',1, 'md')
redis:sadd(RAMBOS..'bot:vipmem:'..msg.chat_id_, msg.sender_user_id_)
redis:del(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(RAMBOS..'karrar1'..msg.sender_user_id_..''..msg.chat_id_..'')
...
if not redis:get(RAMBOS..'rmch1') then
send(msg.chat_id_, msg.id_, 1, '™️┊ لم يتم وضع القناة\n📌┊ لتعين  القناة ارسل .....\n⚠️┊ `تعين قناة الاشتراك `\n➖', 1, 'md')
return false 
end
if redis:get(RAMBOS..'rmch1') then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تفعيل الاشتراك الاجباري\n ✓ ", 1, 'md')
redis:set(RAMBOS.."rmjoin1", true)
return false end end end
...
redis:del(RAMBOS.."dellmssg"..msg.chat_id_)
end
----------------ADD DEL MSG------------------------------------------------------------
...
redis:set(RAMBOS.."dellmssg"..msg.chat_id_, true)
end
----------add - rem bot free----------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تفعيل البوت خدمي\n ✓ ", 1, 'md')
redis:set(RAMBOS.."bot:free", true)
end end
...
local UserRAMBOS = user_info_
if user_info_ then
local start = redis:get(RAMBOS.."startbot")
if start then
keep = [[
[]]..start..[[]
]]
...
💠┊مرحبا انا بوت اسمي *(]]..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..[[)🗼*
...
🔱┊*مطور البوت* » ( []] .. UserRAMBOS .. [[] )‏
‏
‏]]
send(msg.chat_id_, msg.id_, 1, RAMBOS, 1, 'md')
end end end
-----------------------get start------------------------------------------------------------------------
...
local rmch2 = redis:get(RAMBOS.."rmch1")
if rmch2 then
send(msg.chat_id_, msg.id_, 1, '🔱┊ *قناة الاشتراك* : ['..rmch2..']', 1, 'md')
else
...
text = "🌀┊  ادمنيه البوت :\n*≖≖≖≖≖≖≖≖≖≖≖≖*\n"
for k, v in pairs(list) do
if redis:get(RAMBOS.."SudoNumberGp" .. v) then
gps = tonumber(redis:get(RAMBOS.."SudoNumberGp" .. v))
else
gps = 0
end
local user_info = redis:get(RAMBOS.."user:Name" .. v)
...
local time = a[2] * day
redis:setex(RAMBOS.."bot:charge:"..msg.chat_id_,time,true)
redis:set(RAMBOS.."bot:enable:"..msg.chat_id_,true)
end
------------------charge-----------------------------------------------------------------------------
if text:match("^فحص الشحن") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local ex = redis:ttl(RAMBOS.."bot:charge:"..msg.chat_id_)
if ex == -1 then
...
local ex = redis:ttl(RAMBOS.."bot:charge:"..txt[2])
if ex == -1 then
...
if not redis:get(RAMBOS.."bot:free") and not is_sudo(msg) then
send(msg.chat_id_, msg.id_, 1, '🚫┊* للمطوريـــن فقـــط* ❗️', 1, 'md')
return false end
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(RAMBOS..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
if result.members_[i].status_.ID == "ChatMemberStatusCreator" then
owner_id = admins[i].user_id_
redis:sadd(RAMBOS.."bot:owners:"..msg.chat_id_,owner_id)
end
end
end
getChannelMembers(msg.chat_id_, 0, 'Administrators', 200, promote_admin)
redis:set(RAMBOS.."test:group"..msg.chat_id_,'krar')
if redis:get(RAMBOS.."bot:enable:" .. msg.chat_id_) then
...
if not redis:get(RAMBOS.."bot:group:link"..msg.chat_id_) then
local getlink = 'https://api.telegram.org/bot'..RAMBOS_TOKEN..'/exportChatInviteLink?chat_id='..msg.chat_id_
local req = https.request(getlink)
local link = RMJS:decode(req)
if link.ok == true then 
redis:set(RAMBOS.."bot:group:link"..msg.chat_id_,link.result)
end
end
local lik_1 = redis:get(RAMBOS.."bot:group:link"..msg.chat_id_)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(Rm_Owner,0, 1,"- *تم اضافه مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ المدير ≈ [" .. owner .. "]\n🔰┊ ["..title_name(msg.chat_id_).."]("..(link or "t.me/RAMBOS_ch")..")\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " ..result.first_name_.. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n✓", 1, "md")
redis:set(RAMBOS.."bot:enable:" .. msg.chat_id_, true)
redis:setex(RAMBOS.."bot:charge:" .. msg.chat_id_, 9999 * day, true)
redis:sadd(RAMBOS.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
end
tdcli_function ({
ID = "GetChannelFull",
...
redis:set(RAMBOS.."bot:charge:"..txt[2],true)
...
local txt = { string.match(text, "^(تفعيل)$")}
redis:del(RAMBOS..'lock:add'..msg.chat_id_)
local function promote_admin(extra, result, success)
local admins = result.members_
for i=0 , #admins do
redis:sadd(RAMBOS..'bot:momod:'..msg.chat_id_,admins[i].user_id_)
...
redis:set(RAMBOS..'lock:add'..msg.chat_id_,true)
if not redis:get(RAMBOS.."bot:enable:" .. msg.chat_id_) then
...
redis:del(RAMBOS.."bot:enable:" .. msg.chat_id_)
redis:srem(RAMBOS.."bot:groups", msg.chat_id_)
redis:del(RAMBOS.."test:group"..msg.chat_id_)
redis:srem(RAMBOS.."sudo:data:" .. msg.sender_user_id_, msg.chat_id_)
...
send(v,0, 1,"- *تم تعطيل مجموعه* »\nﮧ┉┉┉┉┉┉┉┉┉\n‏🔱┊ المدير ≈ " .. owner .. "\n🔰┊ الرابط ≈ [اضغـط هنـا](" .. (redis:get(RAMBOS.."bot:group:link"..msg.chat_id_) or "https://t.me/RAMBOS_ch") .. ")\n🔅┊* اسم المجموعه* »\n👨🏼┊ﮧ "..title_name(msg.chat_id_).."\n🚫┊ *ايدي المجموعه* »\n📉┊ﮧ "..msg.chat_id_.."\n*«معلومات عن المطور»*\nﮧ┉┉┉┉┉┉┉┉┉\n🌀┊ ايديه ≈ (" .. msg.sender_user_id_ .. ")\n🚫┊ اسمه ≈ " .. fname .. " " .. lname .. "\n️⚠️┊ معرفه ≈ [" .. username .. "]\n‏", 1, "md")
end
...
local gp = txt[3]
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم تفعيل المجموعه \n(*"..txt[2].."*) يوم 🍃", 1, 'html')
redis:sadd(RAMBOS..'sudo:data:'..sudo, gp)
end
-------------------REEM----------------------------------------------------------------------------
...
local send_to_bot_owner = function(extra, result)
redis:del(RAMBOS.."bot:enable:" .. gp[2])
redis:del(RAMBOS.."bot:charge:" .. gp[2])
local v = tonumber(Rm_Owner)
local fname = result.first_name_ or ""
local lname = result.last_name_ or ""
local username = "@" .. result.username_ or ""
...
redis:srem(RAMBOS.."sudo:data:" .. msg.sender_user_id_, gp[2])
redis:srem(RAMBOS.."bot:groups", gp[2])
end
getUser(msg.sender_user_id_, send_to_bot_owner)
end
else
...
local num_keep = (tonumber(redis:get(RAMBOS.."incr_msg"..result.id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(RAMBOS.."msgs:"..result.id_..":"..msg.chat_id_) or  0))
local rmcontact = (tonumber(redis:get(RAMBOS.."rmaddcon"..msg.chat_id_..":"..result.id_) or 0))
if result.id_ then
if tonumber(result.id_) == tonumber(Rm_Owner) then
t = "مطور اساسي 🎖"
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local num_keep = tonumber(redis:get(RAMBOS..'incr_msg'..msg.sender_user_id_..''..msg.chat_id_..'') or 0 )
local msgs = tonumber(redis:get(RAMBOS.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
local rmcontact = (tonumber(redis:get(RAMBOS.."rmaddcon"..msg.chat_id_..":"..msg.sender_user_id_) or 0))
...
if redis:get(RAMBOS.."getidstatus" .. msg.chat_id_) == "Simple" then
send(msg.chat_id_, msg.id_, 1, kepper_info2, 1, "md")
end
if not redis:get(RAMBOS.."getidstatus" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, kepper_info2, 1, "md")
 end 
 end
tdcli_function({
...
local msgs = tonumber(redis:get(RAMBOS.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_))
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "🚫┊ تفاعلك : ( *"..rm_TM_NM(msgs).."* ) 🍁", 1, 'md')
end
end
--------------------rm concat add----------------------------------------
if text == 'جهاتي' then
send(msg.chat_id_, msg.id_, 1, '🔰┊ عدد جهاتك ≈ * '..(tonumber(redis:get(RAMBOS..'rmaddcon'..msg.chat_id_..':'..msg.sender_user_id_) or 0))..' *',1,'md')
end
--------------------ID BY REPLY------------------------------------------
if text:match("^ايدي$") and msg.reply_to_message_id_ ~= 0 then
function iD_reP(extra, result, success)
local num_keep = (tonumber(redis:get(RAMBOS.."incr_msg"..result.sender_user_id_..""..msg.chat_id_.."") or 0 ))
local msgs = (tonumber(redis:get(RAMBOS.."msgs:"..result.sender_user_id_..":"..msg.chat_id_) or 0 ))
local rmcontact = (tonumber(redis:get(RAMBOS.."rmaddcon"..msg.chat_id_..":"..result.sender_user_id_) or 0))
if result.id_ then
if tonumber(result.sender_user_id_) == tonumber(Rm_Owner) then
RAMBOS3 = "مطور الاساسـي 🌿"
elseif is_sudoid(result.sender_user_id_) then
RAMBOS3 = "المطور 🐾"
elseif is_admin(result.sender_user_id_) then
RAMBOS3 = "ادمن في البوت"
elseif is_vipmems(result.sender_user_id_) then
RAMBOS3 = "مميز عام 🌿"
elseif is_monshi(result.sender_user_id_, msg.chat_id_) then
RAMBOS3 = "منشىء الكروب 🐾"
elseif is_owner(result.sender_user_id_, msg.chat_id_) then
RAMBOS3 = "المدير 🌿"
elseif is_momod(result.sender_user_id_, msg.chat_id_) then
RAMBOS3 = "ادمن المجموعه🐾"
...
local RAMBOS_info = "🎟┊ ايديه  » `" .. result.sender_user_id_ .. "`\n👤┊ جهاته  »   "..rmcontact.."\n🔆┊ نقاطه  »   "..num_keep.."\n🌐┊ تفاعله »  " .. rm_TM_NM(msgs) .. "\n✉️┊ رسائله » " .. msgs .. "\n📌┊ موقعه » " .. RAMBOS3 .. "\n‏┄┄┄┄┄┄┄┄┄┄┄┄"
send(msg.chat_id_, result.id_, 1, RAMBOS_info, 1, "md")
end
...
redis:setex(RAMBOS..'botts:namess'..msg.sender_user_id_,698,true)
...
if redis:get(RAMBOS.."getpro:" .. msg.chat_id_) == "Active" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تفعيل جلب الصور\n ✓ ", 1, 'md')
redis:set(RAMBOS.."getpro:" .. msg.chat_id_, "Active")
end end end
...
if redis:get(RAMBOS.."getpro:" .. msg.chat_id_) == "Deactive" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تعطيل جلب الصور\n ✓ ", 1, 'md')
redis:set(RAMBOS.."getpro:" .. msg.chat_id_, "Deactive")
end end end
---------------------------------------------------------------------
if text:match("^مسح الصوره") and is_momod(msg.sender_user_id_, msg.chat_id_)  then
https.request('https://api.telegram.org/bot'..RAMBOS_TOKEN..'/deleteChatPhoto?chat_id='..msg.chat_id_)
...
if not is_momod(msg.sender_user_id_, msg.chat_id_) and redis:get(RAMBOS.."getpro:" .. msg.chat_id_) == "Deactive" then
...
redis:set(RAMBOS..'flood:time:'..msg.chat_id_,floodt[2])
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم وضع التكرار بالطرد\n ✓ ", 1, 'md')
redis:set(RAMBOS.."floodstatus" .. msg.chat_id_, "Kicked")
end
end
if status[2] == "بالمسح" then
if redis:get(RAMBOS.."floodstatus" .. msg.chat_id_) == "DelMsg" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم وضع التكرار بالمسح\n ✓ ", 1, 'md')
redis:set(RAMBOS.."floodstatus" .. msg.chat_id_, "DelMsg")
end
end
end
-------------------SET warn-----------------------------------------
...
redis:set(RAMBOS.."warn:max:" .. msg.chat_id_, warnmax[2])
end
end
---------------------warnstatus----------------------------------------------------------
...
if redis:get(RAMBOS.."warnstatus" .. msg.chat_id_) == "Muteuser" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم وضع التحذير بالكتم\n ✓ ", 1, 'md')
redis:set(RAMBOS.."warnstatus" .. msg.chat_id_, "Muteuser")
end   end
if status[2] == "بالطرد" then
if redis:get(RAMBOS.."warnstatus" .. msg.chat_id_) == "Remove" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم وضع التحذير بالطرد\n ✓ ", 1, 'md')
redis:set(RAMBOS.."warnstatus" .. msg.chat_id_, "Remove")
end end end
-----------------------getidstatus-----------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تفعيل الايدي بالصوره\n ✓ ", 1, 'md')
redis:set(RAMBOS.."getidstatus" .. msg.chat_id_, "Photo")
end end  end
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تعطيل الايدي بالصوره\n ✓ ", 1, 'md')
redis:set(RAMBOS.."getidstatus" .. msg.chat_id_, "Simple")
end end end
-------------------autoleave-----------------------------------------------------------------------
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تفعيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(RAMBOS.."autoleave", "On")
end end end
if is_sudo(msg) and text:match("^تعطيل (.*)$") then
local status = { string.match(text, "^(تعطيل) (.*)$")}
if status[2] == "الخروج التلقائي" then
if redis:get(RAMBOS.."autoleave") == "Off" then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
else
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تعطيل الخروج التلقائي\n ✓ ", 1, 'md')
redis:set(RAMBOS.."autoleave", "Off")
end   end  end
-----------------------------------------------------------
...
redis:setex(RAMBOS.."bot:RAMBOS_dev" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 370, true)
end end
...
local RAMBOS_dev = redis:get(RAMBOS.."RAMBOS_dev")
if RAMBOS_dev then
send(msg.chat_id_, msg.id_, 1, "["..RAMBOS_dev.."]", 1, "md")
else
local karrar = [[
...
redis:setex(RAMBOS.."bot:group:link" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 1200, true)
end
if is_momod(msg.sender_user_id_, msg.chat_id_) and idf:match("-100(%d+)") and (text:match("^حذف الرابط$"))  then
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم حذف الرابط بنجاح\n ✓ ", 1, 'md')
redis:del(RAMBOS.."bot:group:link" .. msg.chat_id_)
end
if text:match("^الرابط$") then
local link = redis:get(RAMBOS.."bot:group:link" .. msg.chat_id_)
if link then
...
local locrmt = {string.match(text, "^([Aa]ction) (.*)$")}
if locrmt[2] == "text" then
sendaction(msg.chat_id_, "Typing")
end
if locrmt[2] == "video" then
sendaction(msg.chat_id_, "RecordVideo")
end
if locrmt[2] == "voice" then
sendaction(msg.chat_id_, "RecordVoice")
end
if locrmt[2] == "photo" then
...
redis:setex(RAMBOS.."Filtering:" .. msg.sender_user_id_, 80, msg.chat_id_)
end
----------------------------------------------------------------------------------
...
local hash = "bot:filters:" .. msg.chat_id_
if not redis:hget(RAMBOS..hash, name) then
send(msg.chat_id_, msg.id_, 1, "🌀┊  الكلمه〖  " .. name .. "  〗\n👥┊  تم الغاء منعها📍", 1, "md")
redis:hdel(RAMBOS..hash, name)
end end
-------------------USERS----------------------------------------------------
if text == 'المشتركين' or text == 'عدد المشتركين' and is_RM(msg) then
local users = redis:scard(RAMBOS.."bot:userss")
local botnamess = redis:get(RAMBOS.."RAMBOSnams") or "كيبر"
...
local get_me = function(extra, result)
local num = (redis:scard(RAMBOS.."bot:groups"))
local list = redis:smembers(RAMBOS.."bot:groups")
local text = "~ All_Groups_Bots \n\n"
for k,v in pairs(list) do
local GroupsOwner = redis:scard(RAMBOS.."bot:owners:"..v) or 0
local GroupsMod = redis:scard(RAMBOS.."bot:momod:"..v) or 0
local Groupslink = redis:get(RAMBOS.."bot:group:link" ..v)
if result.first_name_ then
if #result.first_name_ < 35 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
file:write(text)
file:close()
local token_files = 'https://api.telegram.org/bot' .. RAMBOS_TOKEN .. '/sendDocument'
...
local botnamess = redis:get(RAMBOS.."RAMBOSnams") or "كيبر"
local RmSudos1 = redis:scard(RAMBOS.."Bot:RmSudos") or "0"
local rmAdmins = redis:scard(RAMBOS.."Bot:Admins") or "0"
local rmgban = redis:scard(RAMBOS.."bot:gban:") or "0"
local rmvipmems = redis:scard(RAMBOS.."bot:vipmems:") or "0"
local rmgps = redis:scard(RAMBOS.."bot:groups") or "0"
local rmusers = redis:scard(RAMBOS.."bot:userss") or "0"
send(msg.chat_id_, msg.id_, 1, "*- احصائيات البـــــوت ≈*\n*ﮧ -------------------»*\n👨🏼┊اسم البوت ≈ *" .. botnamess .. "*\n🃏┊عدد المطورين ≈ *"..RmSudos1.."*\n🔰┊عدد الادمنيه ≈ *"..rmAdmins.."*\n🔚┊عدد الكروبات ≈ * "..rmgps.."*\n👪┊عدد المشتركين خاص ≈ *" .. rmusers .. "*\n🔔┊عدد المحظورين عام ≈ *"..rmgban.."*\n🔘┊عدد المميزين عام ≈ * "..rmvipmems.."*\n🗯┊قناة السورس ≈ [RAMBOS](t.me/RAMBOS_ch)\n✓", 1,"md")
end
end
------------------GPS AND USERS-------------------------------------------
if text:match("^المجموعات$") or text:match("^الكروبات$") and is_admin(msg.sender_user_id_, msg.chat_id_) then
local gps = redis:scard(RAMBOS.."bot:groups")
local users = redis:scard(RAMBOS.."bot:userss")
...
function cb(a,b,c)
local text = b.content_.text_
local users = redis:scard(RAMBOS.."bot:userss") or 0
local userss = redis:smembers(RAMBOS.."bot:userss") or 0
local msgs = {
[0] = id
}
for i = 1, #userss do
...
if redis:get(RAMBOS.."ffosh"..msg.chat_id_) and not is_momod(msg.sender_user_id_, msg.chat_id_) then
local id = msg.id_
local msgs = { [0] = id}
local chat = msg.chat_id_
delete_msg(chat, msgs)
...
send(msg.chat_id_, msg.id_, 1, "🚫┊ الرتبه » (*"..RAMBOS3.."*) 🍃\n‏" , 1, "md")
end
...
`git clone https://github.com/alqaser/RAMBOS.git && cd RAMBOS && chmod +x RAMBOS && ./RAMBOS`
...
`killall screen && cd RAMBOS && screen ./RAMBOS`

💬┊ *مطور السورس *≈ [@rr20r](t.me/rr20r)
💲┊ *قناٌة السوٰرس *≈ [@RAMBOS_ch](T.ME/RAMBOS_ch)
...
redis:set(RAMBOS..'welcome:'..msg.chat_id_,welcome)
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم وضع الترحيب\n ✓ ", 1, 'md')
end
-----------------welcome------------------------------------------
if text:match("^الترحيب$") and is_momod(msg.sender_user_id_, msg.chat_id_) then
local wel = redis:get(RAMBOS..'welcome:'..msg.chat_id_)
if wel then
...
redis:setex(RAMBOS.."broadcast" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
redis:setex(RAMBOS.."broadcast2" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
send(msg.chat_id_, msg.id_, 1, "🌀┊  ارسل لي الرساله الان📲\n🚫┊ للخروج ارسل لي الغاء 📍 ", 1, "md")
end end end
----------------- info bot --------------------------------------------------------------
...
local gps = redis:scard(RAMBOS.."bot:groups")
local users = redis:scard(RAMBOS.."bot:userss")
local allmgs = redis:get(RAMBOS.."bot:allmsgs")
if redis:get(RAMBOS.."autoleave") == "On" then
autoleaveAR = "مفعل"
elseif redis:get(RAMBOS.."autoleave") == "Off" then
autoleaveAR = "معطل"
elseif not redis:get(RAMBOS.."autoleave") then
autoleaveAR = "معطل"
end
if redis:get(RAMBOS.."joinbylink") == "On" then
joinbylinkAR = "مفعل"
elseif redis:get(RAMBOS.."joinbylink") == "Off" then
joinbylinkAR = "معطل"
elseif not redis:get(RAMBOS.."joinbylink") then
joinbylinkAR = "معطل"
end
...
Uptime_1 = redis:get(RAMBOS.."bot:Uptime")
...
--------------------------------------------------------------------------------------------------------------
if text == 'وقت تشغيل البوت' and is_RM(msg) then
local cm = io.popen("uptime -p"):read("*all")
local ResultUptimeServer = GetUptimeServer(cm, lang)
if 4 > string.len(ResultUptimeServer) then
if lang == "Ar" then
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local lockRAMBOS = {string.match(text, "^(قفل) (.*)$")}
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيـــــــه فقــــــــط', 1, 'md')
else
if lockRAMBOS[2] == "التعديل" then
if not redis:get(RAMBOS.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التعديل \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS..'editmsg'..msg.chat_id_,'delmsg')
end
if lockRAMBOS[2] == "الاوامر" then
if not redis:get(RAMBOS.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الاوامر \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:cmds" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "البوتات" then
if not redis:get(RAMBOS.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:bots:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "البوتات بالطرد" then
if not redis:get(RAMBOS.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوتات بالطرد \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊البوتات بالطرد مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:botskick" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "التكرار" then
if not redis:get(RAMBOS.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التكرار \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."anti-flood:" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "التثبيت" then
if not redis:get(RAMBOS.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل التثبيت \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مقفول سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:pin:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الكل" then
if not redis:get(RAMBOS.."bot:muteall" .. msg.chat_id_) then
...
redis:set(RAMBOS.."bot:bots:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."anti-flood:" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:photo:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:video:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:selfvideo:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:document:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."markdown:lock" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:gifs:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:music:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:voice:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."tags:lock" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:contact:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:tgservice:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."bot:forward:mute" .. msg.chat_id_, true)
redis:set(RAMBOS.."ffosh"..msg.chat_id_, true)
end
if lockRAMBOS[2] == "الدردشه" then
if not redis:get(RAMBOS.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الدردشه \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:text:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الصور" then
if not redis:get(RAMBOS.."bot:photo:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "المشاركه" then
if not redis:get(RAMBOS.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل المشاركه \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:duplipost:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الكلايش" then
if not redis:get(RAMBOS.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم قفل الكلايش \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:spam:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الفيديو" then
if not redis:get(RAMBOS.."bot:video:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "السيلفي" then
if not redis:get(RAMBOS.."bot:selfvideo:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "الفايلات" then
if not redis:get(RAMBOS.."bot:document:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "الالعاب" then
if not redis:get(RAMBOS.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الالعاب \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."Game:lock" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الانلاين" then
if not redis:get(RAMBOS.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانلاين \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:inline:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "البوست" then
if not redis:get(RAMBOS.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل البوست \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."post:lock" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الماركدون" then
if not redis:get(RAMBOS.."markdown:lock" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "المتحركه" then
if not redis:get(RAMBOS.."bot:gifs:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "الاغاني" then
if not redis:get(RAMBOS.."bot:music:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "الصوت" then
if not redis:get(RAMBOS.."bot:voice:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "الروابط" then
if not redis:get(RAMBOS.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الروابط \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:links:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "المواقع" then
if not redis:get(RAMBOS.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل المواقع \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:location:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "التاك" then
if not redis:get(RAMBOS.."tags:lock" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "الهاشتاك" then
if not redis:get(RAMBOS.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الهاشتاك \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:hashtag:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الجهات" then
if not redis:get(RAMBOS.."bot:contact:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "الويب" then
if not redis:get(RAMBOS.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الويب \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:webpage:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "العربيه" then
if not redis:get(RAMBOS.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل العربيه \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:arabic:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الانظمام" then
if not redis:get(RAMBOS.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانظمام \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:member:lock" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الانكليزيه" then
if not redis:get(RAMBOS.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الانكليزي \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:english:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "الاشعارات" then
if not redis:get(RAMBOS.."bot:tgservice:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "الملصقات" then
if not redis:get(RAMBOS.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم قفل الملصقات \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مقفوله سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:set(RAMBOS.."bot:sticker:mute" .. msg.chat_id_, true)
end
if lockRAMBOS[2] == "التوجيه" then
if not redis:get(RAMBOS.."bot:forward:mute" .. msg.chat_id_) then
...
if lockRAMBOS[2] == "الفشار" then
if not redis:get(RAMBOS.."ffosh" .. msg.chat_id_) then
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local UNRAMBOS = { string.match(text, "^(فتح) (.*)$") }
if not is_momod(msg.sender_user_id_, msg.chat_id_)then
send(msg.chat_id_, msg.id_, 1, '💲┊ للأدمنيـــــــه فقــــــــط', 1, 'md')
else
if UNRAMBOS[2] == "التعديل" then
if redis:get(RAMBOS.."editmsg" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التعديل \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
redis:del(RAMBOS.."editmsg" .. msg.chat_id_)
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التعديل مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
end
if UNRAMBOS[2] == "الاوامر" then
if redis:get(RAMBOS.."bot:cmds" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الاوامر \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الاوامر مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:cmds" .. msg.chat_id_)
end
if UNRAMBOS[2] == "البوتات" then
if redis:get(RAMBOS.."bot:bots:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:bots:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "البوتات بالطرد" then
if redis:get(RAMBOS.."bot:botskick" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوتات بالطرد \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوتات بالطرد مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:botskick" .. msg.chat_id_)
end
if UNRAMBOS[2] == "التكرار" then
if redis:get(RAMBOS.."anti-flood:" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التكرار \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التكرار مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."anti-flood:" .. msg.chat_id_)
end
if UNRAMBOS[2] == "التثبيت" then
if redis:get(RAMBOS.."bot:pin:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح التثبيت \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ التثبيت مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:pin:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الكل" then
if redis:get(RAMBOS.."bot:muteall" .. msg.chat_id_) then
...
redis:del(RAMBOS.."bot:bots:mute" .. msg.chat_id_)
redis:del(RAMBOS.."anti-flood:" .. msg.chat_id_)
redis:del(RAMBOS.."bot:photo:mute" .. msg.chat_id_)
redis:del(RAMBOS.."bot:video:mute" .. msg.chat_id_)
redis:del(RAMBOS.."bot:selfvideo:mute" .. msg.chat_id_)
redis:del(RAMBOS.."bot:document:mute" .. msg.chat_id_)
redis:del(RAMBOS.."markdown:lock" .. msg.chat_id_)
redis:del(RAMBOS.."bot:gifs:mute" .. msg.chat_id_)
redis:del(RAMBOS.."bot:music:mute" .. msg.chat_id_)
redis:del(RAMBOS.."bot:voice:mute" .. msg.chat_id_)
redis:del(RAMBOS.."tags:lock" .. msg.chat_id_)
redis:del(RAMBOS.."bot:contact:mute" .. msg.chat_id_)
redis:del(RAMBOS.."bot:tgservice:mute" .. msg.chat_id_)
redis:del(RAMBOS.."bot:forward:mute" .. msg.chat_id_)
redis:del(RAMBOS.."ffosh"..msg.chat_id_)
end
if UNRAMBOS[2] == "الدردشه" then
if redis:get(RAMBOS.."bot:text:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الدردشه \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الدردشه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:text:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الصور" then
if redis:get(RAMBOS.."bot:photo:mute" .. msg.chat_id_) then
...
if UNRAMBOS[2] == "المشاركه" then
if redis:get(RAMBOS.."bot:duplipost:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح المشاركه \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ المشاركه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:duplipost:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الكلايش" then
if redis:get(RAMBOS.."bot:spam:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1,  "💬┊ تم فتح الكلايش \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1,  "💬┊ الكلايش مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:spam:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الفيديو" then
if redis:get(RAMBOS.."bot:video:mute" .. msg.chat_id_) then
...
if UNRAMBOS[2] == "الالعاب" then
if redis:get(RAMBOS.."Game:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الالعاب \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الالعاب مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."Game:lock" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الانلاين" then
if redis:get(RAMBOS.."bot:inline:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانلاين \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانلاين مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:inline:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "البوست" then
if redis:get(RAMBOS.."post:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح البوست \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")            
else
send(msg.chat_id_, msg.id_, 1, "💬┊ البوست مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."post:lock" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الماركدون" then
if redis:get(RAMBOS.."markdown:lock" .. msg.chat_id_) then
...
if UNRAMBOS[2] == "الروابط" then
if redis:get(RAMBOS.."bot:links:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الروابط \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الروابط مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:links:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "المواقع" then
if redis:get(RAMBOS.."bot:location:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح المواقع \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ المواقع مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:location:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "التاك" then
if redis:get(RAMBOS.."tags:lock" .. msg.chat_id_) then
...
if UNRAMBOS[2] == "الهاشتاك" then
if redis:get(RAMBOS.."bot:hashtag:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الهاشتاك \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الهاشتاك مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:hashtag:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الجهات" then
if redis:get(RAMBOS.."bot:contact:mute" .. msg.chat_id_) then
...
if UNRAMBOS[2] == "الويب" then
if redis:get(RAMBOS.."bot:webpage:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الويب \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الويب مفتوح سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:webpage:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "العربيه" then
if redis:get(RAMBOS.."bot:arabic:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح العربيه \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ العربيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:arabic:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الانظمام" then
if redis:get(RAMBOS.."bot:member:lock" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانظمام \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانظمام مفتوح سابقا\n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:member:lock" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الانكليزيه" then
if redis:get(RAMBOS.."bot:english:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الانكليزي \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الانكليزيه مفتوحه سابقا \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:english:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "الاشعارات" then
if redis:get(RAMBOS.."bot:tgservice:mute" .. msg.chat_id_) then
...
if UNRAMBOS[2] == "الملصقات" then
if redis:get(RAMBOS.."bot:sticker:mute" .. msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "💬┊ تم فتح الملصقات \n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
else
send(msg.chat_id_, msg.id_, 1, "💬┊ الملصقات مفتوحه سابقا\n🎟┊ الأمر بواسطه » "..tmRAMBOS(msg).."\n ‏ ", 1, "md")
end
redis:del(RAMBOS.."bot:sticker:mute" .. msg.chat_id_)
end
if UNRAMBOS[2] == "التوجيه" then
if redis:get(RAMBOS.."bot:forward:mute" .. msg.chat_id_) then
...
if UNRAMBOS[2] == "الفشار" then
if redis:get(RAMBOS.."ffosh"..msg.chat_id_) then
...
redis:del(RAMBOS.."keed_text" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_photo" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_video" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_Document" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_markdon" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_gif" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_audio" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_voice" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_link" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_user" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_hashtag" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_contect" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_arbic" .. msg.chat_id_)
end
...
redis:del(RAMBOS.."keed_fosh" .. msg.chat_id_)
end
...
redis:set(RAMBOS.."bot:sens:spam" .. msg.chat_id_, sensspam[2])
...
local rmbanned = redis:scard(RAMBOS.."bot:banned:" .. msg.chat_id_) or "0"
redis:del(RAMBOS.."bot:banned:" .. msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المحضورين ≈ *"..rmbanned.."*\n🌀┊تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
end end
----------------------------------------
...
local rmgban = redis:scard(RAMBOS.."bot:gban:") or "0"
redis:del(RAMBOS.."bot:gban:")
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المحضورين عام ≈ *"..rmgban.."*\n🌀┊تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
end end
----------------------------------------
...
redis:setex(RAMBOS.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 35, true)
end end
----------------------------------------------
...
local rmmomod = redis:scard(RAMBOS.."bot:momod:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد الأدمنيه ≈ *"..rmmomod.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOS.."bot:momod:" .. msg.chat_id_)
end end
-----------------------------------------------
...
local RmOwners = redis:scard(RAMBOS.."bot:owners:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المدراء ≈ *"..RmOwners.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOS.."bot:owners:" .. msg.chat_id_)
end end
-----------------------------------------------
...
local rmmonshis = redis:scard(RAMBOS.."bot:monshis:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المنشئين ≈ *"..rmmonshis.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOS.."bot:monshis:" .. msg.chat_id_)
end end
--------DEL RAMBOS_SUDO-----------------------------------------
...
local RmSudos1 = redis:scard(RAMBOS.."Bot:RmSudos") or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المطورين ≈ *"..RmSudos1.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
local hash = "Bot:RmSudos"
local list = redis:smembers(RAMBOS..hash)
for k, v in pairs(list) do
local t = tonumber(v)
...
local rmAdmins = redis:scard(RAMBOS.."Bot:Admins") or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد ادمنيه البوت ≈ *"..rmAdmins.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOS.."Bot:Admins")
end end
-----------DEL vipmem-----------------------------------------------
...
local rmvipmem = redis:scard(RAMBOS.."bot:vipmem:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المميزين ≈ *"..rmvipmem.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOS.."bot:vipmem:" .. msg.chat_id_)
end  end
---------------keeed del --------------------------------------------
...
local rmkeed = redis:scard(RAMBOS.."bot:keed:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المقيدين ≈ *"..rmkeed.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
local hash = "bot:keed:" .. msg.chat_id_
local list = redis:smembers(RAMBOS..hash)
for k, v in pairs(list) do
redis:del(RAMBOS.."bot:keed:" .. msg.chat_id_)
HTTPS.request("https://api.telegram.org/bot"..RAMBOS_TOKEN.."/restrictChatMember?chat_id=" ..msg.chat_id_.. "&user_id=" ..v.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
end end end
------------DEL vipmems-------------------------------------------
...
local rmvipmems = redis:scard(RAMBOS.."bot:vipmems:") or "0"
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المميزين عام ≈ *"..rmvipmems.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
redis:del(RAMBOS.."bot:vipmems:")
end end
------------DEL filters-----------------------------------------
...
local rmmuted = redis:scard(RAMBOS.."bot:muted:" .. msg.chat_id_) or "0"
redis:del(RAMBOS.."bot:muted:" .. msg.chat_id_)
send(msg.chat_id_, msg.id_, 1, "📉┊عدد المكتومين ≈ *"..rmmuted.."*\n🌀┊ تـم مسحهم بنجـــــــــــاح ✓\n‏", 1, "md")
end end end
-------------------kickedlist--------------------------------------------
...
if redis:get(RAMBOS.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then
local d = 0
if text:match("^1$") then
redis:del(RAMBOS.."CleanBlockList" .. msg.chat_id_ .. ":" .. msg.sender_user_id_)
for i = 1, NumberReturn do
getChannelMembers(msg.chat_id_, d, "Kicked", 200, kickedlist)
d = d + 200
end
if redis:get(RAMBOS.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(RAMBOS.."flood:max:" .. msg.chat_id_) then
flood_m = 5
else
flood_m = redis:get(RAMBOS.."flood:max:" .. msg.chat_id_)
end
if not redis:get(RAMBOS.."bot:sens:spam" .. msg.chat_id_) then
spam_c = 400
else
spam_c = redis:get(RAMBOS.."bot:sens:spam" .. msg.chat_id_)
end
if redis:get(RAMBOS.."warn:max:" .. msg.chat_id_) then
sencwarn = tonumber(redis:get(RAMBOS.."warn:max:" .. msg.chat_id_))
else
sencwarn = 4
end
if redis:get(RAMBOS.."floodstatus" .. msg.chat_id_) == "DelMsg" then
floodstatus = "المسح"
elseif redis:get(RAMBOS.."floodstatus" .. msg.chat_id_) == "Kicked" then
floodstatus = "الطرد"
elseif not redis:get(RAMBOS.."floodstatus" .. msg.chat_id_) then
floodstatus = "الطرد"
end
if redis:get(RAMBOS.."warnstatus" .. msg.chat_id_) == "Muteuser" then
warnstatus = "الكتم"
elseif redis:get(RAMBOS.."warnstatus" .. msg.chat_id_) == "Remove" then
warnstatus = "الطرد"
elseif not redis:get(RAMBOS.."warnstatus" .. msg.chat_id_) then
warnstatus = "الكتم"
end
if redis:get(RAMBOS.."bot:music:mute" .. msg.chat_id_) then
mute_music = "✅"
...
os.execute('rm -rf RAMBOS.lua') 
os.execute('wget https://raw.githubusercontent.com/alqaser/RAMBOS/master/RAMBOS.lua') 
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم تحديث السورس \n ✓ ", 1, 'md')
dofile('RAMBOS.lua')  
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
redis:setex(RAMBOS.."rules" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 60, true)
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
redis:set(RAMBOS..'bot:setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if redis:get(RAMBOS.."lang:gp:" .. result.id_) then
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserRAMBOS .. "]\n‏", 1, "md")
else
send(result.id_, 0, 1, "🌀┊  تم اخراج البوت 🎐\n🔱┊ راسل المطور للتفعيل 📮\n🔰┊ *المطور* : [" .. UserRAMBOS .. "]\n‏", 1, "md")
end
chat_leave(result.id_, bot_id)
redis:srem(RAMBOS.."bot:groups", result.id_)
else
...
local gp = tonumber(redis:scard(RAMBOS.."bot:userss"))
gps = redis:smembers(RAMBOS.."bot:userss")
...
if not redis:get(RAMBOS.."dellmssg"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
if not redis:get(RAMBOS.."lock_pinn"..msg.chat_id_) and not is_owner(msg.sender_user_id_, msg.chat_id_) then
...
redis:set(RAMBOS.."pinnedmsg" .. msg.chat_id_, msg.reply_to_message_id_)
...
function gp_RAMBOS_info(arg,data)
local list = redis:smembers(RAMBOS.."bot:owners:" .. msg.chat_id_)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(RAMBOS.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
...
local rmmomod = redis:scard(RAMBOS.."bot:momod:" .. msg.chat_id_) or "0"
local rmvipmem = redis:scard(RAMBOS.."bot:vipmem:" .. msg.chat_id_) or "0"
send(msg.chat_id_, msg.id_, 1, "*- احصائيات في المجموعه »*\n📌┊ المدير » [" .. owner .. "]\n🌋┊ عدد الاعضاء » `"..data.member_count_.."`\n🌋┊عدد الادمنيه » `"..data.administrator_count_.."`\n🌋┊عدد المحضورين » `"..data.kicked_count_.."`\n📌┊ المطور » [" .. sudo .. "]\n\n*- احصائيات في البــــوت » *\n📌┊ عدد المنشئين » *"..rmmonshis.."*\n🗯┊ عدد المحضورين » *"..rmbanned.."*\n🗯┊ عدد المــدراء » * "..RmOwners.."*\n🗯┊ عدد المكتومين » * "..rmmuted.."*\n🗯┊ عدد المقيدين » *"..rmkeed.."*\n🗯┊ عدد الادمنيه » * "..rmmomod.."*\n📌┊ عدد المميزين » *"..rmvipmem.."*\n📌┊ اسم البوت » *"..(redis:get(RAMBOS.."RAMBOSnams") or "كيبر").."*\n💠┊ الايدي » (`"..bot_id.."`)\n💠┊ ايدي المجموعه » 👇🏾\n💠┊ﮧ (`" .. msg.chat_id_ .. "`)\n💠┊ اسم المجموعه » 👇🏾\n📌┊ ﮧ ("..title_name(msg.chat_id_)..")\n‏\n", 1,"md")
end
getChannelFull(msg.chat_id_, gp_RAMBOS_info, nil)
end
---------------------add reply in group-------------------------------------
text = msg.content_.text_
if msg.content_.text_ == 'مسح رد' and  is_owner(msg.sender_user_id_, msg.chat_id_) then
redis:set(RAMBOS..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','del_repgp1')
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم مسح الرد بنجاح\n ✓ ", 1, 'md')
redis:del(RAMBOS..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(RAMBOS..'gif_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOS..'voice_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOS..'stecker_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOS..'video_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOS..'text_repgp'..msg.content_.text_..''..msg.chat_id_..'')
redis:del(RAMBOS..'rep_owner'..msg.content_.text_..''..msg.chat_id_..'')
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد' and is_owner(msg.sender_user_id_, msg.chat_id_)  then
redis:set(RAMBOS..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','set_repgp')
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل لي كلمه الرد الان ❗️\n',1, 'md')
return false    end
if msg.content_.text_ then
local content_keep = redis:get(RAMBOS..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
if content_keep == 'set_repgp' then
send(msg.chat_id_, msg.id_, 1, '📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n' ,  1, 'md')
redis:set(RAMBOS..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'','save_repgp')
redis:set(RAMBOS..'addreplaygp:'..msg.sender_user_id_..''..msg.chat_id_..'',msg.content_.text_)
redis:sadd(RAMBOS..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
return false
end
end
-------------------------------------------------------------------------
if  msg.content_.text_ == 'الردود' and is_owner(msg.sender_user_id_, msg.chat_id_) then
local redod = redis:smembers(RAMBOS..'rep_owner'..msg.chat_id_..'')
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(RAMBOS..'add:repgp'..msg.sender_user_id_..''..msg.chat_id_..'')
redis:del(RAMBOS..'gif_repgp'..v..msg.chat_id_)
redis:del(RAMBOS..'voice_repgp'..v..msg.chat_id_)
redis:del(RAMBOS..'stecker_repgp'..v..msg.chat_id_)
redis:del(RAMBOS..'video_repgp'..v..msg.chat_id_)
redis:del(RAMBOS..'text_repgp'..v..msg.chat_id_)
redis:del(RAMBOS..'rep_owner'..msg.chat_id_..'',msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم مسح جميع الردود\n ✓ ", 1, 'md')
return false
end
...
redis:set(RAMBOS.."add:repallt"..msg.sender_user_id_,'del_rep1')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد لمسحها ❗️\n",1, "md")
return false
end
if msg.content_.text_ then
...
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم مسح الرد للكل\n ✓ ", 1, 'md')
redis:del(RAMBOS.."add:repallt"..msg.sender_user_id_)
redis:del(RAMBOS.."gif_repall"..msg.content_.text_)
redis:del(RAMBOS.."voice_repall"..msg.content_.text_)
redis:del(RAMBOS.."stecker_repall"..msg.content_.text_)
redis:del(RAMBOS.."video_repall"..msg.content_.text_)
redis:del(RAMBOS.."text_repall"..msg.content_.text_)
redis:del(RAMBOS.."rep_sudo",msg.content_.text_)
return false
end
end
--------------------------------------------------------------------------
if msg.content_.text_ == 'اضف رد للكل' and is_sudo(msg)  then
redis:set(RAMBOS.."add:repallt"..msg.sender_user_id_,'set_rep')
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل لي كلمه الرد ❗️\n",1, "md")
return false    end
if msg.content_.text_ then
local content_keep = redis:get(RAMBOS.."add:repallt"..msg.sender_user_id_)
if content_keep == 'set_rep' then
send(msg.chat_id_, msg.id_, 1, "📌┊ ارسل جواب الرد قد يكون\n🗯┊ {نص-ملصق-بصمه...} ❗️\n" ,  1, "md")
redis:set(RAMBOS.."add:repallt"..msg.sender_user_id_,'save_rep')
redis:set(RAMBOS.."addreply2:"..msg.sender_user_id_, msg.content_.text_)
redis:sadd(RAMBOS.."rep_sudo",msg.content_.text_)
return false
end    end
------------------------------------------------------------------------------------
if  msg.content_.text_ == "ردود المطور" and is_sudo(msg) then
local redod = redis:smembers(RAMBOS.."rep_sudo")
if #redod == 0 then
...
for k,v in pairs(redod) do
redis:del(RAMBOS.."add:repallt"..v)
redis:del(RAMBOS.."gif_repall"..v)
redis:del(RAMBOS.."voice_repall"..v)
redis:del(RAMBOS.."stecker_repall"..v)
redis:del(RAMBOS.."video_repall"..v)
redis:del(RAMBOS.."text_repall"..v)
redis:del(RAMBOS.."rep_sudo",msg.content_.text_)
end
send(msg.chat_id_, msg.id_, 1, "💬┊ بواسطه » "..tmRAMBOS(msg).."\n🎟┊ تم مسح ردود المطور\n ✓ ", 1, 'md')
return false
...
local list = redis:smembers(RAMBOS.."bot:owners:" .. chattid)
if list[1] or list[2] or list[3] or list[4] then
user_info = redis:get(RAMBOS.."user:Name" .. (list[1] or list[2] or list[3] or list[4]))
end
if user_info then
...
local rmmonshis = redis:scard(RAMBOS.."bot:monshis:" .. chattid) or "0"
local rmbanned = redis:scard(RAMBOS.."bot:banned:" .. chattid) or "0"
local RmOwners = redis:scard(RAMBOS.."bot:owners:" .. chattid) or "0"
local rmmuted = redis:scard(RAMBOS.."bot:muted:" .. chattid) or "0"
local rmkeed = redis:scard(RAMBOS.."bot:keed:" .. chattid) or "0"
local rmmomod = redis:scard(RAMBOS.."bot:momod:" .. chattid) or "0"
local rmvipmem = redis:scard(RAMBOS.."bot:vipmem:" .. chattid) or "0"
if not redis:get(RAMBOS.."bot:group:link"..chattid) then
local getlink = 'https://api.telegram.org/bot'..RAMBOS_TOKEN..'/exportChatInviteLink?chat_id='..chattid
local req = https.request(getlink)
local link = RMJS:decode(req)
if link.ok == true then 
redis:set(RAMBOS.."bot:group:link"..chattid,link.result)
end
end
local lik_1 = redis:get(RAMBOS.."bot:group:link"..chattid)
if lik_1 then
link = lik_1 
else
link = link.result
end
send(msg.chat_id_, msg.id_, 1, "🕴┊ المدير » ["..owner.."]\n🥈┊ﮧ ["..title_name(chattid).."]("..(link or "t.me/RAMBOS_ch")..")\n©️┊ عدد المــــدراء   » *"..RmOwners.."*\n©️┊ عدد المنشئين   » *"..rmmonshis.."*\n🔆┊ عدد الادمنيـــه   » *"..rmmomod.."*\n🔰┊ عدد المكتومين  » *"..rmmuted.."*\n®️┊ عدد المحظورين » *"..rmbanned.."*\n®️┊ عدد المقيديــن   » *"..rmkeed.."*\n🗯┊ عدد المميزيـــن  » *"..rmvipmem.."*\n💠┊ﮧ `"..chattid.."`\n🏮┊ المطور » ["..sudo.."]\n✓", 1,"md")
end
tdcli_function ({
ID = "GetChannelFull",
...
getChannelFull(chattid, gp_RAMBOS_info, nil)
end 
end
------------UNPIN-----------------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
tdcli.sendDocument(RAMBOS_SUDO, 0, 0, 1, nil, './RAMBOS.lua', dl_cb, nil)
...
pinmsg(msg.chat_id_, pin_id, 0)
elseif redis:get(RAMBOS.."lang:gp:" .. msg.chat_id_) then
...
if not redis:get(RAMBOS.."lock_kickme"..msg.chat_id_) then
send(msg.chat_id_, msg.id_, 1, "🌀┊ امر اطردني معطل 🍃", 1, "md")
return false
end
redis:set(RAMBOS.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:set(RAMBOS.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ ارسل *{ نعم }* لطردك\n🚫┊ ارسل *{ لا } *لالغاء طردك", 1, "md")
end
local rmkick = redis:get(RAMBOS.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
if rmkick == "kick_mee" then
...
if rmkick == "kick_mee" then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del(RAMBOS.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:del(RAMBOS.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
send(msg.chat_id_, msg.id_, 1, "🔰┊ تم طردتك حياتي", 1, "md")
end 
end
end
if text:match("^لا$") then
local rmunkick = redis:get(RAMBOS.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."")
if rmunkick == "kick_no" then
redis:del(RAMBOS.."kick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_mee")
redis:del(RAMBOS.."unkick_me"..msg.sender_user_id_..""..msg.chat_id_.."","kick_no")
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
redis:set(RAMBOS.."bot:viewget" .. msg.sender_user_id_, true)
...
redis:set(RAMBOS.."addstart1"..msg.sender_user_id_, "theaddstarts")
return false
end end
if text then
local theaddstart = redis:get(RAMBOS.."addstart1"..msg.sender_user_id_)
if theaddstart == 'theaddstarts' then
send(msg.chat_id_, msg.id_, 1, "🌀┊ تم حفظ الكليشه 🍃", 1, 'md')
redis:del(RAMBOS.."addstart1"..msg.sender_user_id_)
redis:set(RAMBOS.."startbot", text)
return false
end end
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local get_me = function(extra, result)
local msgs = (tonumber(redis:get(RAMBOS.."msgs:"..msg.sender_user_id_..":"..msg.chat_id_)) or "0" )
local rmtext = (tonumber(redis:get(RAMBOS.."text:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local rmsticker = (tonumber(redis:get(RAMBOS.."sticker:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local pkPhoto = (tonumber(redis:get(RAMBOS.."Photo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local rmVoice = (tonumber(redis:get(RAMBOS.."Voice:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local rmGif = (tonumber(redis:get(RAMBOS.."Gif:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local rmVideo = (tonumber(redis:get(RAMBOS.."Video:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local rmSelfVideo = (tonumber(redis:get(RAMBOS.."SelfVideo:"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
local rmcon = (tonumber(redis:get(RAMBOS.."rmaddcon"..msg.sender_user_id_..":"..msg.chat_id_.."")) or "0" )
if result.first_name_ then
if #result.first_name_ < 25 then
else
for kkkkk in string.gmatch(result.first_name_, "[^%s]+") do
...
•- * ⁽ ]]..rmcon..[[ ₎*  📲 جهات
•- * ⁽ ]]..rmsticker..[[ ₎*  🗺 ملصقات
•- * ⁽ ]]..rmSelfVideo..[[ ₎*  📽 فيديو امامي
•- * ⁽ ]]..rmGif..[[ ₎*  🖥 صور متحركـه

⚜️┊العدد الكلي ⌯ *(]]..msgs..[[)* رساله
‌‏
]]
send(msg.chat_id_, msg.id_, 1, rmmsgsss, 1, 'md')
end
getUser(msg.sender_user_id_, get_me)
end end
--------------------------------------------------------
if text == ''..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..' هينه' or text == ''..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..' رزله' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(RAMBOS_SUDO) then
send(msg.chat_id_, msg.id_, 1, 'انجب لك هذا مطوري العشق 😌💋', 1, 'md')
return false  end
if tonumber(result.sender_user_id_) == tonumber(bot_id) then
send(msg.chat_id_, msg.id_, 1, 'لك مگدر اهين نفسي 😞😂', 1, 'md')
return false  end
local RAMBOS = { "لك دايح ، احترم نفسك لا بال 👠😠","ها مصراع تراچي ، اگعد راحه تره روحي طالعه 😐🍃","ها ابن الحنينه، ليش متسكت وتنجب 🌚"}
send(msg.chat_id_, result.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
...
--------------------------------------------------------------------------------------------
if text == ''..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..' بوسه' or text == ''..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..' مصه' then
function reep22(extra, result, success)
if tonumber(result.sender_user_id_) == tonumber(RAMBOS_SUDO) then
...
return false  end
local RAMBOS = { "اععع 🤢خده بي حب شباب الوصخ😹😹","موااح 💋 مواااح  حياتي💋😌🍃","💋😞نسخ لصق ع الشفه 👄"}
send(msg.chat_id_, result.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
if tonumber(msg.reply_to_message_id_) == 0 then
else
...
--------------------------------- rdood bot --------------------------------------------------
if not redis:get(RAMBOS.."lock_reeeep"..msg.chat_id_) then
if text == "السلام عليكم" or text == "سلام عليكم" or text == "سلام" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"وعليكم السلام والرحمه⇣😻","يمه هلا بالغالي 😻🍃","وعليكم السلام حبيبي ☺️🍃","كافي بس تسلمون 🌝💔" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {'اسمي '..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..' 😌👌','تره اسمي '..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..' يالجريذي ☹️😹'  }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
if text == ''..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..'' then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {'اي نعم اني '..(redis:get(RAMBOS..'RAMBOSnams') or 'كيبر')..' 🍃😐','اهو اجوي الملطلطين 😹😪','كافي تره كرهت اسمي 💔😠'}
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-------------------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"هلووووات  ⁽🙆♂✨₎ֆ","يمه هلا بالعافيه 😻🍃","لا هلا ولا مرحبه شلونك مشتاقين 😻😂","اخلاً وصخلا 😌😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"تمام وانت/ي 😘🍃","شعليك انت 🧐😂","بخير انت/ي شلونك/ج ☺️","تمام وانت/ي ‏ ᵛ͢ᵎᵖ💛﴾" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"عـٰٰـٰود لوتُٰي 🙀 يـٰگول بُِوت عبالـٰه طافٰـُٰي💔 ويضِٰل يمٰـٓسلت وينشٰٰر روابـٰٓط 😳🍃","اي بوت شتريد 😤" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"شكلولك عليه كاولي 😶😂","صوتي محلو للاسف 😌💔","اشعجـب كاطع بيه ياراحتي النفسيه 😂💔","حرام  الغنا 😐🍃" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"بايات 💛🍃","گلعه 😶💔","الله الله الله وياك 😻😂","ثيمالا 🌝✋🏾" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"منو ال أجوو👀😹","جااااوات  ₎✿💥😈 ❥" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"نورك/ج هذا ورده 🌝🍃","بوجودك/غلا تسلم 😻✨","انت/ي اصل النور 😋🍃","عماني نورك 😣😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "بوسني" or text == "بوسه" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then local RAMBOS = {"مووووووووواححح💋😻","مابوس ولي😌😹","خدك/ج نضيف 😂🍃","البوسه بالف حمبي 🌝💋" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"عيـــب 🙀😹","شبيه امك حمبي😋🍃" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"شبيه الشكره ام الوصخ 🤭😹","حبيته فدوووه😻","شرايد من خالته 🤭😂","خالته تفلش 😶😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"تف عليك ادبسزز 😒😹","لا تتفل على وجهك 😻😹","ما اسمحلك هيلگ 😡😹","بدون تفال رجائاً 😹😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"كافـي ضحــك 😐","لتضحك هواي وتصير فاول 🌝😹","هذ شبي يضحك 🙀😳","اضحك هيه الدنيا خربانه 😂😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"لصير عصبي يرتفع ضغطـك 😌😂","صار وجه احمر مثل الطماطه 🙊😹","اوف شحلاتك وانت ضايج 😻","شبي هذا الله يستر 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😒" or text == "😒😒" or text == "😒😒😒" or text == "😒😒😒😒" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"لضوج متسوووه 🤭😹","يمه زعلان الحلو ما يكلي مرحبا 😻😹","اعدل وجهك لا اعدله الك/ج 😼👊🏼" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"يمه الگمر عذبني حبه 🙊😻","عو نضيف الوصخ 😹😹","طفي ضواك عميتني 😼😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💋" or text == "💋💋" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"كبر 🙀 جان استحيتو 😹😹","عســـل 😋🙊" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "💔" or text == "💔💔" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"شبي مكسور 😔💔","موجوع كلبي والتعب بيه 😔😹","اكل بصل وانسه الحصل 😻😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😻" or text == "😻😻" or text == "😍😍" or text == "😍" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"شوفو الحب صاعد فول 😻😹","ها ناوي تزحف 😹😹","فدوووه لهاي العيون 🙊😻" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "😐" or text == "🙂" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"شبيك حبيبي 😂💔","منور محمد الاعمى 😐😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"دووم الضحكه 🙊🍃","دوم الضحكه ℡̮⇣┆👑😻⇣ۦ ٰ" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"اي غرد/ي دا اسمع 👂🏽😹","كول😹 (كول لو هدف)😔😹","ها حياتي 🙊" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"انت/ي شبيك/ج😣","مابيه شي تسلم 💋😹" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
----------------------------------------
if text == "🌚💔" or text == "💔" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"هم اجانه محترك وجه😂♥️","هاي منو كاسر كلبك😡","اهو هم اجانه صخام🐸👌" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
---------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"ها بدت حبجيه ✨😂","لتلح عود يعني احبج🙈😹","كافي درينه مشتاقله 😒" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
---------------------------------------
if text == "😢" or text == "😢😢" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"يبجي دلوع😜😹","هاي عود انت جبير كاعد تبجي😑💔","لتلح درينه تبجي😒"}
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"علساس انت متفاعل😒😒","اي عندك اعتراض🤔","اني معليه احرسكم😎" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"عود اني حباب ونت شيطان يتعلم منك🙈😂😂","وجهك ميساعد🤢😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"اهو لا هلا بيه ✨😂","خي ولي مزاعله ✨😂","اهلا بيه بس اطرده اذا اجه😒😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
------------------------------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"وخرو وخرو🤓 هلا بلخميس تيرارا وياي يلا😍😹","هلا بلخميس عطله وكذا ركصو يلا😍😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
-----------------------------------------
if text == "🙊" or text == "🙈" then
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"اول مره اشوف قرد يستحي🤔😂","ما مرجيه منك هايه صاير تستحي انته هوايه 😍😂😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
--------------------------------------
...
if not redis:get(RAMBOS..'lock:add'..msg.chat_id_) then
local RAMBOS = {"ها ها يمعودين احنه هنا😒😹","اعتقد اكو خاص وخطبو وهنا زفه بسيارتي🚗😂" }
send(msg.chat_id_, msg.id_, 1,''..RAMBOS[math.random(#RAMBOS)]..'', 1, 'md')
end
end
end
...
if idf:match("-100(%d+)") and not redis:get(RAMBOS.."bot:muted:Time" .. chat .. ":" .. user) and redis:sismember(RAMBOS.."bot:muted:" .. chat, user) then
redis:srem(RAMBOS.."bot:muted:" .. chat, user)
end
...
if (text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]")) and redis:get(RAMBOS.."bot:links:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.entities_ and result.content_.entities_[0] and (result.content_.entities_[0].ID == "MessageEntityTextUrl" or result.content_.entities_[0].ID == "MessageEntityUrl") and redis:get(RAMBOS.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if result.content_.web_page_ and redis:get(RAMBOS.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if (text:match("[Hh][Tt][Tt][Pp]") or text:match("[Ww][Ww][Ww]") or text:match(".[Cc][Oo]") or text:match(".[Oo][Rr][Gg]") or text:match(".[Ii][Rr]")) and redis:get(RAMBOS.."bot:webpage:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("@") and redis:get(RAMBOS.."tags:lock" .. msg.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("#") and redis:get(RAMBOS.."bot:hashtag:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
end
if text:match("[\216-\219][\128-\191]") and redis:get(RAMBOS.."bot:arabic:mute" .. result.chat_id_) then
local msgs = {
[0] = data.message_id_
}
delete_msg(msg.chat_id_, msgs)
...
if redis:get(RAMBOS..'bot:english:mute'..result.chat_id_) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs)
end
end
if redis:get(RAMBOS..'editmsg'..msg.chat_id_) == 'delmsg' then
local id = msg.message_id_
local msgs = {[0] = id}
local chat = msg.chat_id_
delete_msg(chat,msgs)
elseif redis:get(RAMBOS..'editmsg'..msg.chat_id_) == 'didam' then
if redis:get(RAMBOS..'bot:editid'..msg.message_id_) then
local old_text = redis:get(RAMBOS..'bot:editid'..msg.message_id_)
...
redis:sadd(RAMBOS.."groups:users" .. msg.chat_id_, msg.sender_user_id_)
if text then
...
redis:set(RAMBOS.."Bot:rmBotAccount", data.value_.value_)
end end end end  end
----- End Source By Karrar RAMBOS »»»»»»»»»»»»»»»»»»»
